...
struct ku_ptys_reg {
    ...
    uint32_t ext_eth_proto_lp_advertise;
...
typedef enum sxd_paos_lock_mode {
    ...
    SXD_PAOS_LOCK_MODE_FORCE_DOWN_BY_CONFIG_AT_INIT_E = 0x4,
    SXD_PAOS_LOCK_MODE_LOCKED_AFTER_DOWN_E = 0x8,
    SXD_PAOS_LOCK_MODE_MIN = SXD_PAOS_LOCK_MODE_FORCE_DOWN_BY_FUSE_E,
    SXD_PAOS_LOCK_MODE_MAX = SXD_PAOS_LOCK_MODE_LOCKED_AFTER_DOWN_E
...
typedef enum sxd_ppcnt_grp {
    ...
    SXD_PPCNT_GRP_MAX = SXD_PPCNT_GRP_INFINIBAND_GENERAL_COUNTERS_E
} sxd_ppcnt_grp_t;
...
typedef enum sxd_pmmp_eeprom_override {
    ...
    SXD_PMMP_EEPROM_OVERRIDE_MAX = SXD_PMMP_EEPROM_OVERRIDE_OVERRIDE_CABLE_PROTOCOLS_AND_TECHNOLOGY_FOR_CMIS_E
...
typedef enum sxd_sltp_conf_mod {
    SXD_SLTP_CONF_MOD_TX_7NM_E = 0x0,
    SXD_SLTP_CONF_MOD_TX_7NM_LUT_E = 0x1,
    SXD_SLTP_CONF_MOD_MIN = SXD_SLTP_CONF_MOD_TX_7NM_E,
    SXD_SLTP_CONF_MOD_MAX = SXD_SLTP_CONF_MOD_TX_7NM_LUT_E
} sxd_sltp_conf_mod_t;
...
typedef enum sxd_pddr_neg_mode_active {
    ...
    SXD_PDDR_NEG_MODE_ACTIVE_XDR_AN_NEGOTIATED_E = 0x7,
    SXD_PDDR_NEG_MODE_ACTIVE_MIN = SXD_PDDR_NEG_MODE_ACTIVE_PROTOCOL_WAS_NOT_NEGOTIATED_E,
    SXD_PDDR_NEG_MODE_ACTIVE_MAX = SXD_PDDR_NEG_MODE_ACTIVE_XDR_AN_NEGOTIATED_E
...
#define SXD_PDDR_LINK_DOWN_BUFFER_NUM 3
...
typedef struct sxd_pddr_link_down_buffer {
    uint32_t array512_auto[SXD_PDDR_ARRAY512_AUTO_NUM];
} sxd_pddr_link_down_buffer_t;

typedef struct sxd_pddr_pddr_link_down_info_buffer_page {
    uint8_t buff_sel;
    sxd_pddr_link_down_buffer_t link_down_buffer[SXD_PDDR_LINK_DOWN_BUFFER_NUM];
...
#define SXD_SBSRD_EXT_OCCUPANCY_STATUS_NUM 128
...
typedef enum sxd_upvid_side {
    SXD_UPVID_SIDE_HOST_SIDE_E = 0x1,
    SXD_UPVID_SIDE_LINE_SIDE_E = 0x2,
    SXD_UPVID_SIDE_MIN = SXD_UPVID_SIDE_HOST_SIDE_E,
    SXD_UPVID_SIDE_MAX = SXD_UPVID_SIDE_LINE_SIDE_E
} sxd_upvid_side_t;

/**
 * ku_upvid_reg structure is used to store the UPVID register parameters
 */
struct ku_upvid_reg {
    uint8_t local_port;
    uint8_t lp_msb;
    sxd_upvid_side_t side;
    uint16_t pvid;
};

/**
 * ku_access_upvid_reg structure is used to store the access register UPVID command parameters
 */
struct ku_access_upvid_reg {
    struct ku_operation_tlv op_tlv; /**< op_tlv - operation tlv struct */
    struct ku_upvid_reg     upvid_reg; /**< upvid_reg - upvid register tlv */
    uint8_t                 dev_id; /**< dev_id - device id */
};
...
#define SXD_UPBT_UTCAM_KEY_ID_NUM 4
...
struct ku_upbt_reg {
    ...
    uint16_t region_id;
    uint32_t utcam_key_id[SXD_UPBT_UTCAM_KEY_ID_NUM];
    sxd_upbt_op_t op;
};
...
struct ku_usak_reg {
    uint16_t uengine;
    uint16_t sadb_entry_index;
    uint32_t sa_key[SXD_USAK_SA_KEY_NUM];
};
...
struct ku_uter_reg {
    ...
    uint32_t time_stamp_sec;
    uint32_t time_stamp_nsec;
    uint32_t entry[SXD_UTER_ENTRY_NUM];
};
