                                        ...
                                        u32                     rate)
{
    ...
    psample_sample_packet(group, ci->skb, rate, &md);
}
...
static void __psample_handler(struct completion_info* ci, void *context)
{
    ...
    for (i = 0; i < MAX_PORT_NETDEV_NUM; i++) {
        if (g_netdev_resources->port_netdev[PORT_TYPE_SINGLE][ingress_local_port][i] != NULL) {
            ingress_ifindex = g_netdev_resources->port_netdev[PORT_TYPE_SINGLE][ingress_local_port][i]->ifindex;
            break;
        }
    }
    ...
    if (egress_local_port <= MAX_PHYPORT_NUM) {
        for (i = 0; i < MAX_PORT_NETDEV_NUM; i++) {
            if (g_netdev_resources->port_netdev[PORT_TYPE_SINGLE][egress_local_port][i] != NULL) {
                egress_ifindex = g_netdev_resources->port_netdev[PORT_TYPE_SINGLE][egress_local_port][i]->ifindex;
                break;
            }
        }
    }
...
skip_local_port:
    ...
    __psample_sample_packet_ext(ci, listener_ctx->group, ingress_ifindex,
                                egress_ifindex, rate);
#else
    psample_sample_packet(listener_ctx->group, ci->skb, ci->original_packet_size,
                          ingress_ifindex, egress_ifindex, rate);
#endif
}
