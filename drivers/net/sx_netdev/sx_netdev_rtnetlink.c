...
#endif
{
    ...
    if (!sx_core_if.init_done) {
        ...
        printk(KERN_ERR PFX "%s: can't add new port netdevice during sx_core detach/reset. \n", __func__);
        return -ENXIO;
    }
    read_unlock_irqrestore(&sx_core_if.access_lock, flags);
...
#endif
{
    ...
    if (!sx_core_if.init_done) {
        read_unlock_irqrestore(&sx_core_if.access_lock, flags);
        printk(KERN_ERR PFX "%s: can't change port netdevice during sx_core detach/reset. \n", __func__);
        return -ENXIO;
    }
    read_unlock_irqrestore(&sx_core_if.access_lock, flags);
...
static void sx_netdev_dellink(struct net_device *dev, struct list_head *head)
{
    ...
    if (!sx_core_if.init_done) {
        read_unlock_irqrestore(&sx_core_if.access_lock, flags);
        printk(KERN_ERR PFX "%s: can't delete port netdevice during sx_core detach/reset. \n", __func__);
        return;
    }
    read_unlock_irqrestore(&sx_core_if.access_lock, flags);
