...
int _access_reg_pddr_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_pddr_reg * reg_data)
{
    ...
    case SXD_PDDR_PAGE_SELECT_TROUBLESHOOTING_INFO_PAGE_E:
        ...
        snp_res = snprintf(buffer,
                           buffer_length - buffer_size,
                           "status_message: %s\n",
                           reg_data->page_data.pddr_troubleshooting_page.status_message);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PDDR_PAGE_SELECT_MODULE_INFO_PAGE_E:
        ...
        for (i = 0; i < SXD_PDDR_VENDOR_NAME_NUM; i++) {
            snp_res = snprintf(buffer,
                               buffer_length - buffer_size,
                               "%x",
                               reg_data->page_data.pddr_module_info.vendor_name[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
        ...
        for (i = 0; i < SXD_PDDR_VENDOR_PN_NUM; i++) {
            snp_res = snprintf(buffer,
                               buffer_length - buffer_size,
                               "%x",
                               reg_data->page_data.pddr_module_info.vendor_pn[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
        ...
        for (i = 0; i < SXD_PDDR_VENDOR_SN_NUM; i++) {
            snp_res = snprintf(buffer,
                               buffer_length - buffer_size,
                               "%x",
                               reg_data->page_data.pddr_module_info.vendor_sn[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
    ...
    case SXD_PDDR_PAGE_SELECT_LINK_DOWN_INFO_E:
        ...
        snp_res = snprintf(buffer,
                           buffer_length - buffer_size,
                           "time_to_link_down_to_disable: 0x%x\n",
        ...
        snp_res = snprintf(buffer,
                           buffer_length - buffer_size,
                           "time_to_link_down_to_rx_loss: 0x%x\n",
...
void __sx_netdev_dump_per_dev(struct net_device *netdev)
{
    ...
    printk(KERN_INFO "======   Netdev %s , netdev_p:%p , if_index: %d dump:   =====\n",
           netdev->name, netdev, netdev->ifindex);

    for (uc_type = USER_CHANNEL_L3_NETDEV; uc_type < NUM_OF_NET_DEV_TYPE; uc_type++) {
        printk(KERN_INFO "num of %s traps: %d\n", uc_str[uc_type], net_priv->num_of_traps[uc_type]);
        for (i = 0; i < MAX_NUM_TRAPS_TO_REGISTER; i++) {
            if (net_priv->trap_ids[uc_type][i].synd != SX_INVALID_TRAP_ID) {
                printk(KERN_INFO "trap[%d]: 0x%x\n", i, net_priv->trap_ids[uc_type][i].synd);
                printk(KERN_INFO "Filter_type %s (%d) - Port:%d, lag:%d, vlan:%d\n",
