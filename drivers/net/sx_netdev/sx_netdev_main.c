...
static int sx_netdev_unregister_global_event_handler(void)
{
    ...
    CALL_SX_CORE_FUNC_WITH_RET(sx_core_remove_synd, err, ROUTER_PORT_SWID, NUM_HW_SYNDROMES, L2_TYPE_DONT_CARE, 0,
                               &crit, NULL, g_sx_dev, NULL, NULL, 1);
    if (err) {
...
int sx_netdev_create_port_sysfs_nodes(struct net_device *netdev)
{
    ...
    if (module_support_en && net_priv->is_port_netdev) {
        net_priv->kobj = kobject_create_and_add(netdev->name, &(THIS_MODULE->mkobj.kobj));
        net_priv->kobj_module = kobject_create_and_add("module", net_priv->kobj);
        err = sysfs_create_file(net_priv->kobj_module, &(module_reset_attr.attr));
        if (err) {
            goto out;
        }
        err = sysfs_create_file(net_priv->kobj_module, &(module_power_mode_policy_attr.attr));
        if (err) {
            goto out;
        }
        err = sysfs_create_file(net_priv->kobj_module, &(module_power_mode_attr.attr));
        if (err) {
            goto out;
        }
        err = sysfs_create_file(net_priv->kobj, &(link_ext_state_attr.attr));
        if (err) {
            goto out;
        }
    }
...
int sx_netdev_remove_port_sysfs_nodes(struct net_device *netdev)
{
    ...
    if (module_support_en && net_priv->is_port_netdev) {
        kobject_del(net_priv->kobj_module);
        kobject_del(net_priv->kobj);
    }
                            ...
                            union sx_event_data *event_data)
{
    ...
    case SX_DEV_EVENT_PCI_PRE_RESTART: /* called just before PCI_RESTART */
        g_sx_dev = NULL;
        break;

    case SX_DEV_EVENT_PCI_POST_RESTART: /* called just after PCI_RESTART */
        g_sx_dev = dev;
        break;
...
static void sx_netdev_init_sx_core_interface(void)
{
    ...
    INIT_ONE_SX_CORE_FUNC(sx_core_reset_port_module);
    INIT_ONE_SX_CORE_FUNC(sx_core_set_port_module_power_mode_policy);
    INIT_ONE_SX_CORE_FUNC(sx_core_get_port_module_power_mode);
...
static void sx_netdev_deinit_sx_core_interface(void)
{
    ...
    DEINIT_ONE_SX_CORE_FUNC(sx_core_reset_port_module);
    DEINIT_ONE_SX_CORE_FUNC(sx_core_set_port_module_power_mode_policy);
    DEINIT_ONE_SX_CORE_FUNC(sx_core_get_port_module_power_mode);
}
...
static int __sx_netdev_get_net_device_by_kobj(struct kobject *kobj, struct net_device **dev, bool kobj_module)
{
    int                 ret = 0;
    u8                  i = 0;
    u32                 port = 0;
    ...
    for (port = 0; port < MAX_SYSPORT_NUM; port++) {
        for (i = 0; i < num_netdevs; i++) {
            ...
            if (netdev != NULL) {
                ...
                if (kobj == (kobj_module ? priv->kobj_module : priv->kobj)) {
                    *dev = netdev;
                    goto out;
                }
            }
...
static ssize_t reset_module(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t len)
{
    ...
    ret = __sx_netdev_get_net_device_by_kobj(kobj, &netdev, true);
    if (ret) {
                                              ...
                                              size_t                 len)
{
    ...
    if (ret) {
        printk(KERN_INFO PFX "sysfs entry power_mode_policy got invalid value\n");
        goto out;
    }
...
static ssize_t show_link_ext_state(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    ret = __sx_netdev_get_net_device_by_kobj(kobj, &netdev, false);
    if (ret) {
        printk(KERN_ERR PFX "Failed to get the net device of the kobj\n");
        goto out;
    }
...
static void __bind_unbind_sx_core(bool is_bind, bool is_shutdown_event)
{
    ...
    void          *sx_core_reset_port_module_tmp = NULL;
    void          *sx_core_set_port_module_power_mode_policy_tmp = NULL;
    void          *sx_core_get_port_module_power_mode_tmp = NULL;
    ...
    if (is_bind) {
        ...
        GET_ONE_SX_CORE_FUNC(sx_core_reset_port_module);
        GET_ONE_SX_CORE_FUNC(sx_core_set_port_module_power_mode_policy);
        GET_ONE_SX_CORE_FUNC(sx_core_get_port_module_power_mode);
        ...
        ASSIGN_ONE_FUNC_PTR(sx_core_reset_port_module);
        ASSIGN_ONE_FUNC_PTR(sx_core_set_port_module_power_mode_policy);
        ASSIGN_ONE_FUNC_PTR(sx_core_get_port_module_power_mode);
        ...
        EMPTY_ONE_FUNC_PTR(sx_core_reset_port_module);
        EMPTY_ONE_FUNC_PTR(sx_core_set_port_module_power_mode_policy);
        EMPTY_ONE_FUNC_PTR(sx_core_get_port_module_power_mode);
        ...
        PUT_ONE_SX_CORE_FUNC(sx_core_reset_port_module);
        PUT_ONE_SX_CORE_FUNC(sx_core_set_port_module_power_mode_policy);
        PUT_ONE_SX_CORE_FUNC(sx_core_get_port_module_power_mode);
    }
}
