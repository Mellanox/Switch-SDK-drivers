...
static int sk_bfd_socket_bindtodevice(struct socket *sock, char *linux_vrf_name)
{
    ...
    if (err < 0) {
        printk(KERN_WARNING "Failed to set BFD socket SO_BINDTODEVICE option (vrf:%s). Err = %d\n",
               linux_vrf_name, err);
    }
                          ...
                          char            *linux_vrf_name)
{
    ...
    if (err < 0) {
        printk(KERN_ERR "Failed to create BFD socket (err:%d) .\n", err);
        return -EIO;
    }
    ...
    if (err < 0) {
        printk(KERN_ERR "Failed to bind BFD socket to port %d (err:%d).\n",
               port, err);
        return -EIO;
    }
    ...
    err = t_sock->ops->setsockopt(t_sock,
                                  ...
                                  sizeof(value));
    if (err < 0) {
        printk(KERN_WARNING "Failed to set BFD socket IP_RECVTTL option (err:%d).\n",
               err);
    }
    ...
    err = t_sock->ops->setsockopt(t_sock,
                                  ...
                                  sizeof(value));
    if (err < 0) {
        printk(KERN_WARNING "Failed to set BFD socket IPV6_RECVHOPLIMIT option (err:%d).\n",
               err);
    }
    ...
    err = t_sock->ops->setsockopt(t_sock,
                                  ...
                                  sizeof(value));
    if (err < 0) {
        printk(KERN_WARNING "Failed to set BFD socket IP_PKTINFO option (err:%d).\n",
               err);
    }
    ...
    err = t_sock->ops->setsockopt(t_sock,
                                  ...
                                  sizeof(value));
    if (err < 0) {
        printk(KERN_WARNING "Failed to set BFD socket IPV6_RECVPKTINFO option (err:%d).\n",
               err);
    }
    ...
    if (err < 0) {
#endif
        printk(KERN_WARNING "Failed to set BFD socket SO_RCVTIMEO option (err:%d).\n",
               err);
    }
    ...
    if (err) {
        printk(KERN_WARNING "setting %d-byte socket receive buffer failed (err:%d)",
               rcvbuf, err);
    }
#else
    err = sock_setsockopt(t_sock, SOL_SOCKET, SO_RCVBUFFORCE,
                            ...
                            char            * linux_vrf_name)
{
    ...
    if (use_vrf_device) {
        ...
        if (err < 0) {
            printk(KERN_ERR "Failed to bind tx BFD socket to vrf device: %s (err:%d).\n",
                   linux_vrf_name, err);
            err = -EIO;
            goto bail;
        }
    ...
    if (err < 0) {
        if (sa_family == AF_INET) {
            printk(KERN_ERR "Failed to bind BFD socket to local_addr (ip:%pI4 ,port:%d) (err:%d).\n",
                   &(((struct sockaddr_in*)local_addr)->sin_addr),
                   ntohs(((struct sockaddr_in*)local_addr)->sin_port),
                   err);
        } else {
            printk(KERN_ERR "Failed to bind BFD socket to local_addr (ip:%pI6 ,port:%d) (err:%d).\n",
...
void * sx_bfd_rx_vrf_init(uint32_t vrf_id, uint8_t use_vrf_device, char *linux_vrf_name, unsigned long bfd_pid)
{
    ...
    if (!entry_vrf) {
        printk(KERN_ERR "RX entry session VRF allocation failed.\n");
        err = -ENOMEM;
        goto bail;
    }
    ...
    err = sx_bfd_rx_socket_init(SX_BFD_SINGLEHOP_PORT,
                                ...
                                linux_vrf_name);
    if (err) {
        printk(KERN_ERR "Create SX_BFD_SINGLEHOP_PORT socket (vrf_d:%d,bfd_pid:%lu,vrf_name:%s) failed (err:%d).\n",
    ...
    err = sx_bfd_rx_socket_init(SX_BFD_MULTIHOP_PORT,
                                ...
                                linux_vrf_name);
    if (err) {
        printk(KERN_ERR "Create SX_BFD_MULTIHOP_PORT socket (vrf_d:%d,bfd_pid:%lu,vrf_name:%s) failed (err:%d).\n",
               vrf->vrf_id, bfd_pid, linux_vrf_name, err);
        err = -EIO;
        goto bail;
    }
    ...
    if (!g_initialized) {
        ...
        if (bfd_thread == NULL) {
            ...
            if (IS_ERR(bfd_thread)) {
                ...
                printk(KERN_ERR "Kernel BFD single-hop thread failed to start (err=%d).\n", err);
                goto bail;
            }
        }
        hash_init(rx_vrfs);
