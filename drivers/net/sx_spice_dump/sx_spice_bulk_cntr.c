...
static int __sx_spice_bulk_cntr_init_mocs(struct sx_dev                  *dev,
                                          struct ku_access_mocs_reg      *reg_data,
                                          ...
                                          enum sxd_bulk_cntr_key_type_e   key_type)
{
    ...
    /* Initialize everything except 'entry' */
    sx_cmd_set_op_tlv(&reg_data->op_tlv, MLXSW_MOCS_ID, 2);

    if (ev_id) {
        reg_data->mocs_reg.event_tid = ev_id->event_id_value;
    } else { /* in 'cancel' operation */
        reg_data->mocs_reg.event_tid = 0ull;
    }

    reg_data->dev_id = dev->device_id;
    reg_data->mocs_reg.clear = clear;
    reg_data->mocs_reg.opcode = opcode;

    err = __sx_spice_bulk_cntr_get_mocs_type_id(key_type, &reg_data->mocs_reg.type);
    if (err) {
        printk(KERN_ERR "%s: invalid argument for key_type_e %d err: %d", __func__,
               key_type, err);
        goto out;
                            ...
                            spice_bulk_cntr_ku_reg_u     *reg_data)
{
    ...
    switch (key_type) {
    case SXD_BULK_CNTR_KEY_TYPE_PERFORMANCE_E:
        err = __MOPCE_decode((u8*)reg_tlv, &reg_data->mopce_reg.mopce_reg, NULL);
        if (err) {
            err = -EFAULT;
...
static int __sx_spice_bulk_cntr_tr_set(spice_bulk_cntr_event_params_t *params)
{
    ...
    memset(&reg_data, 0, sizeof(struct ku_access_mocs_reg));
    ...
    /*send IFC */
    err = sx_ACCESS_REG_MOCS(dev, &reg_data);
    if (err) {
                                    ...
                                    void            *context)
{
    ...
    if (cmd == CTRL_CMD_ADD_SYND) {
        ...
        err = sx_core_remove_synd(ku.swid, ku.syndrome_num,
                                  ...
                                  &critireas, context, dev, NULL, &(ku.port_vlan_params), ku.is_register);
    }
