...
def spice_tree_index_sets_arr_init_func_impl_gen():
    ...
    pattern2 = """\tindex_set_%s_%s_%s_arr_init();\n"""
    ...
    for protocol in INDEX_SET_DB:
        for idx1, idx2 in INDEX_SET_DB[protocol]:
            ...
            code += pattern2 % (protocol, prefix1, prefix2)
...
def dynamic_tree_bool_dec_generate(node):
    """
    Genrates dynamic tree boolean variables declaration
    :param node: dynamic tree node
    :return: dynamic tree boolean variables declaration code.
    """
    bool_dec_pattern = """%sbool is_%s_supported = false;\n"""
    ...
    for child in node:
        if child.tag == 'enum_index' or child.tag == 'seq_index':
            ...
            code += bool_dec_pattern % (INDENT, str)
...
def dynamic_tree_code_generate(protocol, node, parent, nest_level):
    ...
    """
    system_bool_func_call_pattern = """%serr = is_%s_dir_supported(dev_data, &is_%s_supported);
%sif (err) {
%sgoto allocation_failed;
%s}\n
"""
    index_bool_seq_idx_func_call_pattern = """%serr = is_%s_dir_supported(dev_data, %s_it, &is_%s_supported);
%sif (err) {
%sgoto allocation_failed;
%s}\n
"""
    index_bool_enum_idx_func_call_pattern = """%serr = is_%s_dir_supported(dev_data, %s_arr_g[%s_it].field_value), &is_%s_supported);
%sif (err) {
%sgoto allocation_failed;
%s}\n
"""
    if_clause_pattern = """%sif (is_%s_supported) {
%s %s}"""
    else_if_clause_pattern = """ else if (is_%s_supported) {
%s %s}"""
    ...
    for child in node:
        if child.tag == 'enum_index' or child.tag == 'seq_index':
            ...
            if (node.get('type') == 'system'):
                func_call_seg_code += system_bool_func_call_pattern % (nest_level * INDENT, str, str,
                                                                       nest_level * INDENT, (nest_level + 1) * INDENT, nest_level * INDENT)
            elif (node.get('type') == 'index'):
                index_str = node.get('index')
                if parent.tag == 'seq_index':
                    func_call_seg_code += index_bool_seq_idx_func_call_pattern % (nest_level * INDENT, str, index_str, str,
                                                                                  nest_level * INDENT, (nest_level + 1) * INDENT,
                                                                                  nest_level * INDENT)
                elif parent.tag == 'enum_index':
                    parent_str = parent.get('name')
                    if parent.get('reg') != "":
                        parent_str += parent.get('reg')
                    func_call_seg_code += index_bool_enum_idx_func_call_pattern % (nest_level * INDENT, str, parent_str, index_str, str,
                                                                                   nest_level * INDENT, (nest_level + 1) * INDENT,
                                                                                   nest_level * INDENT)
            else:
            ...
            if (first_child):
                dynamic_tree_code += if_clause_pattern % (nest_level * INDENT, str, code, nest_level * INDENT)
                first_child = False
            else:
                dynamic_tree_code += else_if_clause_pattern % (str, code, nest_level * INDENT)
        elif (child.tag == 'reg'):
            code = spice_tree_create_rec(protocol, child, parent, nest_level + 1)
            dynamic_tree_code += else_clause_pattern % (code, nest_level * INDENT)

    return (func_call_seg_code + dynamic_tree_code)
...
%s
\tprintk(KERN_INFO \"sx_spice_tree_%s_%s_create\\n\");

\tsnprintf(dname, sizeof(dname), \"ASIC.%sd\", dev_data->dev->device_id);
\tasic_p = debugfs_create_dir(dname, %s_p);
\tif (!asic_p) {
\t\tgoto allocation_failed;
\t}

\tdev_data->%s_root_inode_p = asic_p;
...
"""
    ...
    # Generates tree booleans declaration.
    for dynamic_tree in tree_node.iter('dynamic_tree'):
        dec_code += dynamic_tree_bool_dec_generate(dynamic_tree)
    ...
    code = pattern % (tree_name, protocol, dec_code, tree_name, protocol, mod_sign, app_name, tree_name, code)
...
"""
    ...
    code = pattern % (tree_name, tree_name, tree_name, tree_name)
