...
int sx_spice_access_reg_pddr_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_pddr_reg * reg_data)
{
    ...
    case SXD_PDDR_PAGE_SELECT_PHY_INFO_PAGE_E:
        ...
        for (i = 0; i < SXD_PDDR_KR_STARTUP_DEBUG_INDICATIONS_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "kr_startup_debug_indications[%d]: 0x%x\n", i, reg_data->page_data.pddr_phy_info_page.kr_startup_debug_indications[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
    ...
    case SXD_PDDR_PAGE_SELECT_MODULE_INFO_PAGE_E:
        ...
        for (i = 0; i < SXD_PDDR_VENDOR_NAME_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "vendor_name[%d]: 0x%x\n", i, reg_data->page_data.pddr_module_info.vendor_name[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
        ...
        for (i = 0; i < SXD_PDDR_VENDOR_PN_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "vendor_pn[%d]: 0x%x\n", i, reg_data->page_data.pddr_module_info.vendor_pn[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
        ...
        for (i = 0; i < SXD_PDDR_VENDOR_SN_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "vendor_sn[%d]: 0x%x\n", i, reg_data->page_data.pddr_module_info.vendor_sn[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
    ...
    case SXD_PDDR_PAGE_SELECT_LINK_DOWN_INFO_BUFFER_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "buff_sel: 0x%x\n", reg_data->page_data.pddr_link_down_info_buffer_page.buff_sel);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        for (i = 0; i < SXD_PDDR_LINK_DOWN_BUFFER_NUM; i++) {
            for (j = 0; j < SXD_PDDR_ARRAY512_AUTO_NUM; j++) {
                snp_res = snprintf(buffer, buffer_length - buffer_size, "array512_auto[%d]: 0x%x\n", j, reg_data->page_data.pddr_link_down_info_buffer_page.link_down_buffer[i].array512_auto[j]);
                VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
                buffer += snp_res;
                buffer_size += snp_res;
            }
