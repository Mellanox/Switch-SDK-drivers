...
int sx_spice_access_reg_pmdr_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_pmdr_reg * reg_data)
{
...
int sx_spice_access_reg_mvcap_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_mvcap_reg * reg_data)
{
    int buffer_size = 0, snp_res = 0;

    snp_res = snprintf(buffer, buffer_length - buffer_size, "sensor_map: 0x%llx\n", reg_data->mvcap_reg.sensor_map);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
...
int sx_spice_access_reg_mtmp_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_mtmp_reg * reg_data)
{
    int buffer_size = 0, snp_res = 0;
...
int sx_spice_access_reg_pddr_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_pddr_reg * reg_data)
{
    int buffer_size = 0, snp_res = 0;
    uint32_t i = 0;
    uint16_t local_port = 0;
    ...
    case SXD_PDDR_PAGE_SELECT_PHY_INFO_PAGE_E:
        ...
        for (i = 0; i < SXD_PDDR_KR_STARTUP_DEBUG_INDICATIONS_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->pddr_reg.page_data.pddr_phy_info_page.kr_startup_debug_indications[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
    ...
    case SXD_PDDR_PAGE_SELECT_MODULE_INFO_PAGE_E:
        ...
        for (i = 0; i < SXD_PDDR_VENDOR_NAME_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->pddr_reg.page_data.pddr_module_info.vendor_name[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
        ...
        for (i = 0; i < SXD_PDDR_VENDOR_PN_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->pddr_reg.page_data.pddr_module_info.vendor_pn[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
        ...
        for (i = 0; i < SXD_PDDR_VENDOR_SN_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->pddr_reg.page_data.pddr_module_info.vendor_sn[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
    ...
    case SXD_PDDR_PAGE_SELECT_PORT_EVENTS_PAGE_E:
        ...
        for (i = 0; i < SXD_PDDR_CLR_MASK_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->pddr_reg.page_data.pddr_port_events_page_layout.clr_mask[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
    ...
    case SXD_PDDR_PAGE_SELECT_DEVICE_EVENTS_PAGE_E:
        ...
        buffer_size += snp_res;
        for (i = 0; i < SXD_PDDR_CLR_MASK_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->pddr_reg.page_data.pddr_device_events_page.clr_mask[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
...
int sx_spice_access_reg_sllm_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_sllm_reg * reg_data)
{
    int buffer_size = 0, snp_res = 0;
    uint16_t local_port = 0;

    snp_res = snprintf(buffer, buffer_length - buffer_size, "status: 0x%x\n", reg_data->sllm_reg.status);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "version: 0x%x\n", reg_data->sllm_reg.version);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    SX_PORT_BUILD_PHY_ID_FROM_LSB_MSB(local_port, reg_data->sllm_reg.local_port, reg_data->sllm_reg.lp_msb);
    snp_res = snprintf(buffer, buffer_length - buffer_size, "local_port: 0x%x\n", local_port);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "pnat: 0x%x\n", reg_data->sllm_reg.pnat);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane: 0x%x\n", reg_data->sllm_reg.lane);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "port_type: 0x%x\n", reg_data->sllm_reg.port_type);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "c_db: 0x%x\n", reg_data->sllm_reg.c_db);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    switch (reg_data->sllm_reg.version) {
    case SXD_SLLM_VERSION_PROD_16NM_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "lm_active: 0x%x\n", reg_data->sllm_reg.page_data.sllm_16nm.lm_active);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "lm_was_active: 0x%x\n", reg_data->sllm_reg.page_data.sllm_16nm.lm_was_active);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "pib_gw_lock: 0x%x\n", reg_data->sllm_reg.page_data.sllm_16nm.pib_gw_lock);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "lm_en: 0x%x\n", reg_data->sllm_reg.page_data.sllm_16nm.lm_en);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "lm_clk90_fl_err_max: 0x%x\n", reg_data->sllm_reg.page_data.sllm_16nm.lm_clk90_fl_err_max);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "lm_clk90_fl_err_acc: 0x%x\n", reg_data->sllm_reg.page_data.sllm_16nm.lm_clk90_fl_err_acc);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ib3_max_lm_90_tries: 0x%x\n", reg_data->sllm_reg.page_data.sllm_16nm.ib3_max_lm_90_tries);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "lm_counter_up: 0x%x\n", reg_data->sllm_reg.page_data.sllm_16nm.lm_counter_up);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ib1_max_lm_90_tries: 0x%x\n", reg_data->sllm_reg.page_data.sllm_16nm.ib1_max_lm_90_tries);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "lm_counter_mid: 0x%x\n", reg_data->sllm_reg.page_data.sllm_16nm.lm_counter_mid);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "lm_counter_dn: 0x%x\n", reg_data->sllm_reg.page_data.sllm_16nm.lm_counter_dn);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
...
int sx_spice_access_reg_sltp_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_sltp_reg * reg_data)
{
    int buffer_size = 0, snp_res = 0;
    uint16_t local_port = 0;

    snp_res = snprintf(buffer, buffer_length - buffer_size, "status: 0x%x\n", reg_data->sltp_reg.status);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "version: 0x%x\n", reg_data->sltp_reg.version);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    SX_PORT_BUILD_PHY_ID_FROM_LSB_MSB(local_port, reg_data->sltp_reg.local_port, reg_data->sltp_reg.lp_msb);
    snp_res = snprintf(buffer, buffer_length - buffer_size, "local_port: 0x%x\n", local_port);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "pnat: 0x%x\n", reg_data->sltp_reg.pnat);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_policy: 0x%x\n", reg_data->sltp_reg.tx_policy);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "conf_mod: 0x%x\n", reg_data->sltp_reg.conf_mod);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane: 0x%x\n", reg_data->sltp_reg.lane);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "lane_speed: 0x%x\n", reg_data->sltp_reg.lane_speed);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "port_type: 0x%x\n", reg_data->sltp_reg.port_type);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "c_db: 0x%x\n", reg_data->sltp_reg.c_db);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    switch (reg_data->sltp_reg.version) {
    case SXD_SLTP_VERSION_PROD_40NM_E:
        break;

    case SXD_SLTP_VERSION_PROD_28NM_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "polarity: 0x%x\n", reg_data->sltp_reg.page_data.sltp_28nm_40nm.polarity);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ob_tap0: 0x%x\n", reg_data->sltp_reg.page_data.sltp_28nm_40nm.ob_tap0);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ob_tap1: 0x%x\n", reg_data->sltp_reg.page_data.sltp_28nm_40nm.ob_tap1);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ob_tap2: 0x%x\n", reg_data->sltp_reg.page_data.sltp_28nm_40nm.ob_tap2);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ob_leva: 0x%x\n", reg_data->sltp_reg.page_data.sltp_28nm_40nm.ob_leva);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ob_preemp_mode: 0x%x\n", reg_data->sltp_reg.page_data.sltp_28nm_40nm.ob_preemp_mode);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ob_reg: 0x%x\n", reg_data->sltp_reg.page_data.sltp_28nm_40nm.ob_reg);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ob_bias: 0x%x\n", reg_data->sltp_reg.page_data.sltp_28nm_40nm.ob_bias);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ob_norm: 0x%x\n", reg_data->sltp_reg.page_data.sltp_28nm_40nm.ob_norm);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "ob_bad_stat: 0x%x\n", reg_data->sltp_reg.page_data.sltp_28nm_40nm.ob_bad_stat);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
...
int sx_spice_access_reg_mgir_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_mgir_reg * reg_data)
{
    int buffer_size = 0, snp_res = 0;
...
int sx_spice_access_reg_ppcnt_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_ppcnt_reg * reg_data)
{
    int buffer_size = 0, snp_res = 0;
    uint32_t i = 0;
    uint16_t local_port = 0;
    ...
    case SXD_PPCNT_GRP_ETHERNET_EXTENDED_COUNTERS_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_int_cksm_err_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.eth_extended_cntrs_grp_data_layout.tx_int_cksm_err_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "tx_int_cksm_err_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.eth_extended_cntrs_grp_data_layout.tx_int_cksm_err_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PPCNT_GRP_RS_FEC_HISTOGRAM_GROUP_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "hist: 0llx");
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        for (i = 0; i < SXD_PPCNT_HIST_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "%llx", reg_data->ppcnt_reg.counter_set.ppcnt_rs_fec_histograms_counters.hist[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
    ...
    case SXD_PPCNT_GRP_INFINIBAND_PKTS_COUNTERS_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "time_since_last_clear_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.time_since_last_clear_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "time_since_last_clear_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.time_since_last_clear_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_unicast_xmit_pkts_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_unicast_xmit_pkts_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_unicast_xmit_pkts_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_unicast_xmit_pkts_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_unicast_rcv_pkts_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_unicast_rcv_pkts_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_unicast_rcv_pkts_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_unicast_rcv_pkts_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "sync_header_error_counter_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.sync_header_error_counter_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "sync_header_error_counter_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.sync_header_error_counter_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_local_physical_errors_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_local_physical_errors_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_local_physical_errors_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_local_physical_errors_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_malformed_packet_errors_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_malformed_packet_errors_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_malformed_packet_errors_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_malformed_packet_errors_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_buffer_overrun_errors_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_buffer_overrun_errors_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_buffer_overrun_errors_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_buffer_overrun_errors_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_dlid_mapping_errors_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_dlid_mapping_errors_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_dlid_mapping_errors_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_dlid_mapping_errors_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_vl_mapping_errors_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_vl_mapping_errors_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_vl_mapping_errors_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_vl_mapping_errors_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_looping_errors_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_looping_errors_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_looping_errors_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_looping_errors_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_inactive_discards_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_inactive_discards_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_inactive_discards_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_inactive_discards_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_neighbor_mtu_discards_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_neighbor_mtu_discards_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_neighbor_mtu_discards_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_packets_counter.port_neighbor_mtu_discards_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
    ...
    case SXD_PPCNT_GRP_INFINIBAND_GENERAL_COUNTERS_E:
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_rcv_data_qword_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_general_counter.port_rcv_data_qword_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_rcv_data_qword_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_general_counter.port_rcv_data_qword_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_xmit_data_qword_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_general_counter.port_xmit_data_qword_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "port_xmit_data_qword_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_general_counter.port_xmit_data_qword_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rq_general_error_high: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_general_counter.rq_general_error_high);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "rq_general_error_low: 0x%x\n", reg_data->ppcnt_reg.counter_set.ppcnt_infiniband_general_counter.rq_general_error_low);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        break;
...
int sx_spice_access_reg_mdir_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_mdir_reg * reg_data)
{
    int buffer_size = 0, snp_res = 0;
    uint32_t i = 0;
    ...
    for (i = 0; i < SXD_MDIR_DEVICE_ID_NUM; i++) {
        snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->mdir_reg.device_id[i]);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    }
...
int sx_spice_access_reg_spzr_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_spzr_reg * reg_data)
{
    int buffer_size = 0, snp_res = 0;
...
int sx_spice_access_reg_mtcap_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_mtcap_reg * reg_data)
{
    int buffer_size = 0, snp_res = 0;
...
int sx_spice_access_reg_ptys_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_ptys_reg * reg_data)
{
    int buffer_size = 0, snp_res = 0;
    uint16_t local_port = 0;
...
int sx_spice_access_reg_spad_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_spad_reg * reg_data)
{
    int buffer_size = 0, snp_res = 0;
...
int sx_spice_access_reg_msgi_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_msgi_reg * reg_data)
{
    int buffer_size = 0, snp_res = 0;
    uint32_t i = 0;
    ...
    for (i = 0; i < SXD_MSGI_SERIAL_NUMBER_NUM; i++) {
        snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->msgi_reg.serial_number[i]);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    }
    ...
    for (i = 0; i < SXD_MSGI_PART_NUMBER_NUM; i++) {
        snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->msgi_reg.part_number[i]);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    }
    ...
    for (i = 0; i < SXD_MSGI_PRODUCT_NAME_NUM; i++) {
        snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->msgi_reg.product_name[i]);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    }
...
ssize_t sx_spice_access_reg_mtmp_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    sxd_params.reg_id = MLXSW_MTMP_ID;
    sx_cmd_set_op_tlv(&reg_data.op_tlv, MLXSW_MTMP_ID, 1);
    reg_data.dev_id = data->dev_data->dev->device_id;
    reg_data.mtmp_reg.i = data->i;
    reg_data.mtmp_reg.ig = data->ig;
    reg_data.mtmp_reg.asic_index = data->asic_index;
    reg_data.mtmp_reg.slot_index = data->slot_index;
    reg_data.mtmp_reg.sensor_index = data->sensor_index;
...
ssize_t sx_spice_access_reg_sllm_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    ssize_t                   print_data_size = 0;
    sxd_params_t              sxd_params;
    struct ku_access_sllm_reg reg_data;
    ...
    memset(kbuffer, 0, sizeof(char) * KERNEL_BUFFER_SIZE);
    memset(&reg_data, 0, sizeof(struct ku_access_sllm_reg));

    sxd_params.reg_id = MLXSW_SLLM_ID;
    sx_cmd_set_op_tlv(&reg_data.op_tlv, MLXSW_SLLM_ID, 1);
    reg_data.dev_id = data->dev_data->dev->device_id;

    SX_PORT_EXTRACT_LSB_MSB_FROM_PHY_ID(reg_data.sllm_reg.local_port,
                                        reg_data.sllm_reg.lp_msb,
                                        data->local_port);
    reg_data.sllm_reg.pnat = data->pnat;
    reg_data.sllm_reg.lane = data->lane;
    reg_data.sllm_reg.port_type = data->port_type;
...
ssize_t sx_spice_access_reg_sltp_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    ssize_t                   print_data_size = 0;
    sxd_params_t              sxd_params;
    struct ku_access_sltp_reg reg_data;
    ...
    memset(kbuffer, 0, sizeof(char) * KERNEL_BUFFER_SIZE);
    memset(&reg_data, 0, sizeof(struct ku_access_sltp_reg));

    sxd_params.reg_id = MLXSW_SLTP_ID;
    sx_cmd_set_op_tlv(&reg_data.op_tlv, MLXSW_SLTP_ID, 1);
    reg_data.dev_id = data->dev_data->dev->device_id;

    SX_PORT_EXTRACT_LSB_MSB_FROM_PHY_ID(reg_data.sltp_reg.local_port,
                                        reg_data.sltp_reg.lp_msb,
                                        data->local_port);
    reg_data.sltp_reg.pnat = data->pnat;
    reg_data.sltp_reg.lane = data->lane;
    reg_data.sltp_reg.port_type = data->port_type;
...
ssize_t sx_spice_access_reg_pphcr_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    ssize_t                    print_data_size = 0;
    sxd_params_t               sxd_params;
    struct ku_access_pphcr_reg reg_data;
    ...
    memset(kbuffer, 0, sizeof(char) * KERNEL_BUFFER_SIZE);
    memset(&reg_data, 0, sizeof(struct ku_access_pphcr_reg));

    sxd_params.reg_id = MLXSW_PPHCR_ID;
    sx_cmd_set_op_tlv(&reg_data.op_tlv, MLXSW_PPHCR_ID, 1);
    reg_data.dev_id = data->dev_data->dev->device_id;

    SX_PORT_EXTRACT_LSB_MSB_FROM_PHY_ID(reg_data.pphcr_reg.local_port,
                                        reg_data.pphcr_reg.lp_msb,
                                        data->local_port);
    reg_data.pphcr_reg.pnat = data->pnat;
    reg_data.pphcr_reg.hist_type = data->hist_type;
...
ssize_t sx_spice_access_reg_ppcnt_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    reg_data.ppcnt_reg.grp_profile = data->grp_profile;
...
ssize_t sx_spice_access_reg_mopce_read(struct file *filp, char *buf, size_t lbuf, loff_t *ppos)
{
    ...
    ssize_t                    print_data_size = 0;
    sxd_params_t               sxd_params;
    struct ku_access_mopce_reg reg_data;
    ...
    memset(kbuffer, 0, sizeof(char) * KERNEL_BUFFER_SIZE);
    memset(&reg_data, 0, sizeof(struct ku_access_mopce_reg));

    sxd_params.reg_id = MLXSW_MOPCE_ID;
    sx_cmd_set_op_tlv(&reg_data.op_tlv, MLXSW_MOPCE_ID, 1);
    reg_data.dev_id = data->dev_data->dev->device_id;
