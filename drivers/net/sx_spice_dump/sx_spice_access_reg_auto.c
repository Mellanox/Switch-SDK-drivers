...
int sx_spice_access_reg_pphcr_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_pphcr_reg * reg_data)
{
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    for (index = 0; index < SXD_PPHCR_BIN_RANGE_NUM; index++) {
        snp_res = snprintf(buffer, buffer_length - buffer_size, "high_val: 0x%x\n", reg_data->pphcr_reg.bin_range[index].high_val);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "low_val: 0x%x\n", reg_data->pphcr_reg.bin_range[index].low_val);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    }
...
int sx_spice_access_reg_ppcnt_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_ppcnt_reg * reg_data)
{
    ...
    case SXD_PPCNT_GRP_RS_FEC_HISTOGRAM_GROUP_E:
        ...
        for (index = 0; index < SXD_PPCNT_HIST_NUM; index++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "%llx", reg_data->ppcnt_reg.counter_set.ppcnt_rs_fec_histograms_counters.hist[index]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
...
int sx_spice_access_reg_mopce_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_mopce_reg * reg_data)
{
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    for (index = 0; index < SXD_MOPCE_PERFORMANCE_COUNTER_INFO_NUM; index++) {
        snp_res = snprintf(buffer, buffer_length - buffer_size, "hw_unit_instance: 0x%x\n", reg_data->mopce_reg.performance_counter_info[index].hw_unit_instance);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "hw_unit_id: 0x%x\n", reg_data->mopce_reg.performance_counter_info[index].hw_unit_id);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "counter_index: 0x%x\n", reg_data->mopce_reg.performance_counter_info[index].counter_index);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "hw_unit_sub_instance: 0x%x\n", reg_data->mopce_reg.performance_counter_info[index].hw_unit_sub_instance);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "counter_value: 0x%llx\n", reg_data->mopce_reg.performance_counter_info[index].counter_value);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    }
