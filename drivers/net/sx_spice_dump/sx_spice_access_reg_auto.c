...
int __spzr_reg_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_access_spzr_reg * reg_data)
{
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "node_description: 0x");
    buffer += snp_res;
    buffer_size += snp_res;
    for (index = 0; index < SXD_SPZR_NODE_DESCRIPTION_NUM; index++) {
        snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->spzr_reg.node_description[index]);
        buffer += snp_res;
        buffer_size += snp_res;
    }
    snp_res = snprintf(buffer, buffer_length - buffer_size, "\n");
