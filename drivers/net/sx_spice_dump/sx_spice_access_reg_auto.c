...
int sx_spice_access_reg_pddr_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_pddr_reg * reg_data)
{
    ...
    case SXD_PDDR_PAGE_SELECT_OPERATIONAL_INFO_PAGE_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "phy_manager_link_width_enabled: 0x%x\n", reg_data->page_data.pddr_operation_info_page.pm_le.pddr_phy_manager_link_enabed_ib.phy_manager_link_width_enabled);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "phy_manager_link_proto_enabled: 0x%x\n", reg_data->page_data.pddr_operation_info_page.pm_le.pddr_phy_manager_link_enabed_ib.phy_manager_link_proto_enabled);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "phy_manager_link_eth_enabled: 0x%x\n", reg_data->page_data.pddr_operation_info_page.pm_le.pddr_phy_manager_link_enabed_eth.phy_manager_link_eth_enabled);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "core_to_phy_link_width_enabled: 0x%x\n", reg_data->page_data.pddr_operation_info_page.cp_le.pddr_c2p_link_enabed_ib.core_to_phy_link_width_enabled);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "core_to_phy_link_proto_enabled: 0x%x\n", reg_data->page_data.pddr_operation_info_page.cp_le.pddr_c2p_link_enabed_ib.core_to_phy_link_proto_enabled);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "core_to_phy_link_eth_enabled: 0x%x\n", reg_data->page_data.pddr_operation_info_page.cp_le.pddr_c2p_link_enabed_eth.core_to_phy_link_eth_enabled);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    ...
    case SXD_PDDR_PAGE_SELECT_PHY_INFO_PAGE_E:
        ...
        buffer += snp_res;
        buffer_size += snp_res;
        snp_res = snprintf(buffer, buffer_length - buffer_size, "kr_startup_fsm_lane: 0x%02x%02x%02x%02x\n", reg_data->page_data.pddr_phy_info_page.kr_startup_fsm_lane[0], reg_data->page_data.pddr_phy_info_page.kr_startup_fsm_lane[1], reg_data->page_data.pddr_phy_info_page.kr_startup_fsm_lane[2], reg_data->page_data.pddr_phy_info_page.kr_startup_fsm_lane[3]);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
        ...
        for (i = 0; i < SXD_PDDR_KR_STARTUP_DEBUG_INDICATIONS_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->page_data.pddr_phy_info_page.kr_startup_debug_indications[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
    ...
    case SXD_PDDR_PAGE_SELECT_MODULE_INFO_PAGE_E:
        ...
        for (i = 0; i < SXD_PDDR_VENDOR_NAME_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->page_data.pddr_module_info.vendor_name[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
        ...
        for (i = 0; i < SXD_PDDR_VENDOR_PN_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->page_data.pddr_module_info.vendor_pn[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
        ...
        for (i = 0; i < SXD_PDDR_VENDOR_SN_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->page_data.pddr_module_info.vendor_sn[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
    ...
    case SXD_PDDR_PAGE_SELECT_PORT_EVENTS_PAGE_E:
        ...
        for (i = 0; i < SXD_PDDR_CLR_MASK_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->page_data.pddr_port_events_page_layout.clr_mask[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
    ...
    case SXD_PDDR_PAGE_SELECT_DEVICE_EVENTS_PAGE_E:
        ...
        buffer_size += snp_res;
        for (i = 0; i < SXD_PDDR_CLR_MASK_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->page_data.pddr_device_events_page.clr_mask[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
...
int sx_spice_access_reg_ppcnt_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_ppcnt_reg * reg_data)
{
    ...
    case SXD_PPCNT_GRP_RS_FEC_HISTOGRAM_GROUP_E:
        ...
        for (i = 0; i < SXD_PPCNT_HIST_NUM; i++) {
            snp_res = snprintf(buffer, buffer_length - buffer_size, "%llx", reg_data->counter_set.ppcnt_rs_fec_histograms_counters.hist[i]);
            VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
            buffer += snp_res;
            buffer_size += snp_res;
        }
...
int sx_spice_access_reg_mdir_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_mdir_reg * reg_data)
{
    ...
    for (i = 0; i < SXD_MDIR_DEVICE_ID_NUM; i++) {
        snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->device_id[i]);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    }
...
int sx_spice_access_reg_ptys_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_ptys_reg * reg_data)
{
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "eth_proto_lp_legacy_valid: 0x%x\n", reg_data->eth_proto_lp_legacy_valid);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
    ...
    buffer += snp_res;
    buffer_size += snp_res;
    snp_res = snprintf(buffer, buffer_length - buffer_size, "eth_proto_lp_legacy_advertise: 0x%x\n", reg_data->eth_proto_lp_legacy_advertise);
    VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
    buffer += snp_res;
    buffer_size += snp_res;
...
int sx_spice_access_reg_msgi_data_to_buffer_print(char buffer[], size_t buffer_length, struct ku_msgi_reg * reg_data)
{
    ...
    for (i = 0; i < SXD_MSGI_SERIAL_NUMBER_NUM; i++) {
        snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->serial_number[i]);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    }
    ...
    for (i = 0; i < SXD_MSGI_PART_NUMBER_NUM; i++) {
        snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->part_number[i]);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    }
    ...
    for (i = 0; i < SXD_MSGI_PRODUCT_NAME_NUM; i++) {
        snp_res = snprintf(buffer, buffer_length - buffer_size, "%x", reg_data->product_name[i]);
        VALIDATE_ROLLBACK(snp_res, buffer_length, buffer_size);
        buffer += snp_res;
        buffer_size += snp_res;
    }
