...
int sx_spice_tree_ber_eth_create(dev_private_data_t *dev_data)
{
    ...
    int             i_it = 0;
    bool            is_ib_sel_supported = false;
    bool            is_fom_measurment_supported = false;
        ...
        for (port_type_it = 0; port_type_it < 6; port_type_it++) {
            ...
            for (lane_it = 0; lane_it <= 7; lane_it++) {
                if (index_set_eth_port_type_lane_arr_g[port_type_it][lane_it]) {
                    ...
                    err = is_ib_sel_dir_supported(dev_data, &is_ib_sel_supported);
                    if (err) {
                        goto allocation_failed;
                    }

                    if (is_ib_sel_supported) {
                    ...
                    err = is_fom_measurment_dir_supported(dev_data, &is_fom_measurment_supported);
                    if (err) {
                        goto allocation_failed;
                    }

                    if (is_fom_measurment_supported) {
...
int sx_spice_tree_ber_ib_create(dev_private_data_t *dev_data)
{
    ...
    err = sx_spice_access_reg_spzr_file_create(asic_p, dev_data);
    if (err) {
        goto allocation_failed;
    }
    for (local_port_it = 1; local_port_it <= dev_data->max_local_port; local_port_it++) {
