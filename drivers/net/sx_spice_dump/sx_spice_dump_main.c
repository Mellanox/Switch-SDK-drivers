...
static void * sx_spice_register(struct sx_dev *dev)
{
    ...
    sx_dbg_dump_read_handler_register("spice_stats", __spice_stats_dump, NULL, priv_data);
...
static void sx_spice_unregister(struct sx_dev *dev, void *private_data)
{
    ...
    sx_dbg_dump_read_handler_unregister("spice_stats");
...
static int __sx_spice_pll_group_num_get(dev_private_data_t *priv_data)
{
    ...
    memset(&reg_data, 0, sizeof(struct ku_ppll_reg));

    err = SX_SPICE_EMAD_ACCESS_REG(PPLL, priv_data, &reg_data);
    if (err) {
        printk(KERN_ERR "SPICE: Failed to get num of pll groups from PPLL register\n");
        goto out;
    }

    priv_data->max_pll_group = reg_data.num_pll_groups;
