...
static int __global_tx_type = HWTSTAMP_TX_OFF;
static u8  __global_tx_msg_type_one_step_bitmap = 0; /* message type bitmap of which time stamp should not be
                                                      *  delivered in socket error queue */
...
int sx_ptp_tx_handler_spc2(struct sx_priv *priv, struct sk_buff *skb, const struct sx_ptp_packet_metadata *pkt_meta)
{
    ...
    if ((1 << pkt_meta->pkt_fields.msg_type) & __global_tx_msg_type_one_step_bitmap) {
...
int sx_ptp_dump_spc2(struct sx_priv *priv, struct seq_file *m, void *v, void *context)
{
    ...
    for (msg_type = 0; msg_type < 16; msg_type++) {
        if ((1 << msg_type) & __global_tx_msg_type_one_step_bitmap) {
    ...
    seq_printf(m, "TX stepping mode: %s\n", __tx_type_to_str(__global_tx_type));
...
int sx_ptp_set_tx_type_spc2(struct sx_priv *priv, int tx_type)
{
    ...
    if (tx_type == __global_tx_type) {
        goto out;
    }

    printk(KERN_NOTICE "PTP stepping mode is changing from %s to %s\n",
           __tx_type_to_str(__global_tx_type), __tx_type_to_str(tx_type));
    ...
    __global_tx_type = tx_type;
    __global_tx_msg_type_one_step_bitmap = tx_msg_type_one_step_bitmap;
