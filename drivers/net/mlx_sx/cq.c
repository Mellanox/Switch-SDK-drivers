           ...
           u8                            dev_id)
{
...
#if defined (PD_BU) && defined (SPECTRUM4_BU)
    /* WA for HW bug 2431837 (on Griffin): HW sends TAC aggregated packet with invalid sysport.
     *  Changing it to be 0 like for events */
    if (ci->sysport == 0xffff) {
        printk(KERN_NOTICE "changing sysport for synd [0x%x] from 0xffff to 0. \n", ci->hw_synd);
        ci->sysport = 0;
    }
#endif
    ...
    if (rx_debug &&
        ...
        printk(KERN_DEBUG PFX "rx_skb: mirror_tclass:%d, mirror_cong:%d, mirror_latency:%d \n",
    ...
    case PKT_TYPE_FCoETH:
        ...
        if (ci->info.eth.ethtype == ETHTYPE_EMAD) {
            ...
            if (ci->info.eth.emad_tid == 0xffffffff) {
                printk(KERN_DEBUG PFX "emad_tid =  0x%x HEALTH CHECK packet via "
...
static int post_skb(struct sx_dq *dq)
{
    ...
    if (!new_skb) {
        ...
        sx_health_external_report_all(SXD_HEALTH_SEVERITY_FATAL, "RDQ entry allocation");
        err = -ENOMEM;
        goto out;
    }
    ...
    if (skb_put(new_skb, size) == NULL) {
        ...
        sx_health_external_report_all(SXD_HEALTH_SEVERITY_FATAL, "RDQ entry set buffer size");
        err = -ENOMEM;
        goto out;
    }
    ...
    if (cqe_params.is_send) {
        ...
        do {
            ...
            if (!found) {
                ...
                dq->sge[idx].skb = NULL;
                if (curr_pkt->completion_delay_reported) {
                    dq->pkts_no_recv_completion--;
    ...
    for (i = cq->cons_index & 0xffff; i != cq_context.producer_counter; i = (i + 1) & 0xffff) {
        ...
        if (enable_monitor_rdq_trace_points) {
            ...
            if (priv->filter_ebpf_progs[dqn]) {
                should_drop = sx_core_call_rdq_filter_trace_point_func(priv->filter_ebpf_progs[dqn],
                                                                       skb,
                                                                       &cqe_params,
                                                                       rdq_max_buff_size);
            } else {
