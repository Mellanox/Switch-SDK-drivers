...
static ssize_t show_asic_temp_highest(struct kobject *kobj, struct kobj_attribute *attr, char *buf);
static ssize_t show_asic_temp_input(struct kobject *kobj, struct kobj_attribute *attr, char *buf);
static ssize_t show_asic_temp_label(struct kobject *kobj, struct kobj_attribute *attr, char *buf);
static struct kobj_attribute asic_temp_highest_attr = __ATTR(highest, S_IRUGO, show_asic_temp_highest, NULL);
static struct kobj_attribute asic_temp_input_attr = __ATTR(input, S_IRUGO, show_asic_temp_input, NULL);
static struct kobj_attribute asic_temp_label_attr = __ATTR(label, S_IRUGO, show_asic_temp_label, NULL);
...
static int __sx_core_init_mtmp(struct sx_dev *dev, uint8_t slot_id, uint16_t sensor_index)
{
    int                       err = 0;
    struct ku_access_mtmp_reg mtmp_reg_data;
...
static bool __sx_core_init_temp(struct sx_dev *dev, void *context)
{
    int                        err = 0;
    ...
    err = __sx_core_init_mtmp(dev, 0, sensor_idx);
    if (err) {
...
static bool __sx_core_create_asic_sysfs(struct sx_dev *dev, void *context)
{
    int             err = 0;
    struct sx_priv *priv = sx_priv(dev);
    ...
    if (priv->kobj[SX_KOBJECT_ASIC_TEMP] == NULL) {
        ...
        err = -ENOMEM;
        goto fail_on_create_sysfs_node;
    }
    ...
    err = sx_core_create_default_modules_sysfs_eeprom_tree(priv->kobj[SX_KOBJECT_ASIC], &(priv->module_eeprom_arr));
    if (err) {
...
static void __sx_core_init_per_asic(void)
{
    sx_dev_db_iterate(__sx_core_create_asic_sysfs, NULL);
    sx_dev_db_iterate(__sx_core_init_temp, NULL);

    return;
}

static bool __sx_core_remove_asic_sysfs(struct sx_dev *dev, void *context)
{
    struct sx_priv *priv = sx_priv(dev);
...
static void __sx_core_cleanup_per_asic(void)
{
    sx_dev_db_iterate(__sx_core_remove_asic_sysfs, NULL);
