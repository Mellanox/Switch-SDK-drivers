...
static void __listeners_gc_free(struct rcu_head *rcu)
{
    struct listener_entry *listener = container_of(rcu, struct listener_entry, rcu);

    kfree(listener);
}

static void __listeners_gc_add(struct listener_entry *listener)
{
    list_del_rcu(&listener->list);
    call_rcu(&listener->rcu, __listeners_gc_free);
}
