                     ...
                     u8                          is_register)
{
    ...
    if (!list_empty(&info->per_synd_list[hw_synd])) {
        list_for_each(pos, &info->per_synd_list[hw_synd]) {
                        ...
                        u8                          is_register)
{
    ...
    if (!list_empty(&(info->per_synd_list[entry]))) {
        list_for_each_safe(pos, q, &info->per_synd_list[entry]) {
            listener = list_entry(pos, struct listener_entry, list);
