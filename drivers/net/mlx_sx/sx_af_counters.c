 ...
 * Functions
 ***********************************************/
extern int send_trap(const void *buf, const uint32_t buf_size, uint16_t trap_id, u8 is_from_user,
                     u8 device_id, pid_t target_pid, gfp_t gfp_mask);
...
static void __sx_af_counters_dump(struct seq_file *m, void *v);
...
static inline void __sx_af_counters_stats_reg_dump(struct seq_file *m, void *v, sx_af_reg_stats_t *af_reg_stats_p);
static void __sx_af_counters_stats_dump(struct seq_file *m, void *v);
...
static inline int __sx_af_counters_ctx_init(struct ku_accuflow_counters_params *af_params_p)
{
    ...
    g_af_counters_ctx.af_counters_wq_p = create_singlethread_workqueue("accuflow_wq");
...
out:
    if (err) {
        if (is_wq_initialized) {
            destroy_workqueue(g_af_counters_ctx.af_counters_wq_p);
...
static void __sx_af_counters_free_handler_cb(struct work_struct *work_p)
{
    ...
    index = __sx_af_counters_prm_index_to_zero_based(af_work_p->data.af_params.op_data.free_data.prm_counter_idx);
    if (index >= __sx_af_counters_prm_indices_num_get()) {
        printk(KERN_ERR PFX "Failed to free a counter because of invalid index %u [%u, %u]\n",
               index, af_work_p->data.af_params.op_data.free_data.prm_counter_idx,
               g_af_counters_ctx.prm_cntr_start_idx_offset);
        goto out;
    }

    for (; index < af_work_p->data.af_params.op_data.free_data.num_of_counters; index++) {
        g_af_counters_ctx.af_counters_states_p[index] = SX_AF_COUNTER_STATE_FLUSH_WIP_E;
        g_af_counters_ctx.af_counters_free_time_p[index] = now;
    }
...
static void __sx_af_counters_clear_handler_cb(struct work_struct *work_p)
{
    ...
    index = __sx_af_counters_prm_index_to_zero_based(af_work_p->data.af_params.op_data.clear_data.prm_counter_idx);
    if (index >= __sx_af_counters_prm_indices_num_get()) {
        printk(KERN_ERR PFX "Failed to clear a counter because of invalid index %u [%u, %u]\n",
               index, af_work_p->data.af_params.op_data.clear_data.prm_counter_idx,
               g_af_counters_ctx.prm_cntr_start_idx_offset);
        goto out;
    }
    ...
    for (;
         index < af_work_p->data.af_params.op_data.clear_data.num_of_counters;
         index++, counter_p++) {
        counter_p->flow_counter_bytes = 0;
        counter_p->flow_counter_packets = 0;
    }
...
static void __sx_af_counters_read_handler_cb(struct work_struct *work_p)
{
    ...
    index = __sx_af_counters_prm_index_to_zero_based(af_work_p->data.af_params.op_data.read_data.prm_counter_idx);
    if (index >= __sx_af_counters_prm_indices_num_get()) {
        printk(KERN_ERR PFX "Failed to read a counter because of invalid index %u [%u, %u]\n",
               index, af_work_p->data.af_params.op_data.read_data.prm_counter_idx,
               g_af_counters_ctx.prm_cntr_start_idx_offset);
        goto out;
    }
    counter_p = (sxd_flow_counter_set_t *)&g_af_counters_ctx.af_counters_buffer_p[index];
    ...
    for (;
         index < af_work_p->data.af_params.op_data.read_data.num_of_counters;
         index++, counter_p++) {
        af_work_p->data.af_params.op_data.read_data.counters_values[index].flow_counter_bytes =
            counter_p->flow_counter_bytes;
        af_work_p->data.af_params.op_data.read_data.counters_values[index].flow_counter_packets =
...
static void __sx_af_counters_mafti_handler_cb(struct work_struct *work_p)
{
    ...
    kfree_skb(af_work_p->data.skb_p);
    kfree(af_work_p);
}
                           ...
                           void                                      *context)
{
    ...
    for (index = 0; index < num_rec; index++) {
        counter_index = __sx_af_counters_prm_index_to_zero_based(mlxsw_reg_mafti_counter_index_get(mafti->reg, index));
        ...
        bytes_inc = mlxsw_reg_mafti_bytes_inc_get(mafti->reg, index);
        packets_inc = mlxsw_reg_mafti_packets_inc_get(mafti->reg, index);
        ...
        counter_p->flow_counter_bytes += bytes_inc * g_af_counters_ctx.bytes_inc_unit +
                                         64 * packets_inc * g_af_counters_ctx.packets_inc_unit;
                           ...
                           void                                      *context)
{
    ...
    for (index = 0; index < num_rec; index++) {
        counter_index = __sx_af_counters_prm_index_to_zero_based(mlxsw_reg_mafbi_entry_mafbi_entry_type0_counter_index_get(
                                                                     mafbi->reg,
                                                                     index));
        if (counter_index >= __sx_af_counters_prm_indices_num_get()) {
            printk(KERN_ERR PFX "counter_index %u exceeds number of counters %u \n",
        ...
        bytes_inc = mlxsw_reg_mafbi_entry_mafbi_entry_type0_bytes_inc_get(mafbi->reg, index);
        packets_inc = mlxsw_reg_mafbi_entry_mafbi_entry_type0_packets_inc_get(mafbi->reg, index);
...
static void __sx_af_counters_dump(struct seq_file *m, void *v)
{
    ...
    seq_printf(m, "\tNumber of counters           : %u\n", g_af_counters_ctx.af_counters_num);
    __sx_af_counters_ktime_dump(m, v, "\tTime now                     :", &time_now, "\n");
    ...
    for (;
         index < g_af_counters_ctx.af_counters_num;
         index++, counter_p++) {
        ...
        __sx_af_counters_ktime_dump(m, v, " ", &g_af_counters_ctx.af_counters_free_time_p[index], "time;");
...
int sx_af_counters_dump(struct seq_file *m, void *v)
{
    ...
    if (!__sx_af_counters_context_acquire()) {
        __sx_af_counters_dump(m, v);
        __sx_af_counters_context_release();
    }

    seq_printf(m, "\n");

    __sx_af_counters_stats_dump(m, v);

    seq_printf(m, "\n");

    sx_af_dbg_info_dump(m, v);
...
static void __sx_af_counters_stats_refresh_dump(struct seq_file *m, void *v, sx_af_refresh_stats_t *af_refresh_stats_p)
{
    seq_printf(m, "\t\tcanceled                 : %u \n", af_refresh_stats_p->canceled);
    __sx_af_counters_ktime_dump(m, v, "\t\tstart                    :", &af_refresh_stats_p->start, "\n");
    __sx_af_counters_ktime_dump(m, v, "\t\tend                      :", &af_refresh_stats_p->end, "\n");
    __sx_af_counters_ktime_dump(m, v, "\t\tduration                 :", &af_refresh_stats_p->duration, "\n");
...
static void __sx_af_counters_stats_reg_dump(struct seq_file *m, void *v, sx_af_reg_stats_t *af_reg_stats_p)
{
...
static void __sx_af_counters_stats_dump(struct seq_file *m, void *v)
{
    ...
    __sx_af_counters_stats_refresh_dump(m, v, &g_af_stats.refresh_all.last_refresh);

    seq_printf(m, "\tShortest refresh:\n");
    __sx_af_counters_stats_refresh_dump(m, v, &g_af_stats.refresh_all.min_refresh);

    seq_printf(m, "\tLongest refresh:\n");
    __sx_af_counters_stats_refresh_dump(m, v, &g_af_stats.refresh_all.max_refresh);

    seq_printf(m, "\tEvent jobs:\n");
    __sx_af_counters_ktime_dump(m, v, "\t\ttimer                    :", &g_af_stats.event_jobs.timer, "\n");
    ...
    seq_printf(m, "\t\tMafti:\n");
    __sx_af_counters_stats_reg_dump(m, v, &g_af_stats.regs.mafti);
    seq_printf(m, "\t\tMafbi:\n");
    __sx_af_counters_stats_reg_dump(m, v, &g_af_stats.regs.mafbi);
}
