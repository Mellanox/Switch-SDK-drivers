...
static void __gc_db(struct sx_priv *priv, int sysport, struct ptp_db *db, bool gc_all)
{
    ...
    list_for_each_entry_safe(iter_common, tmp_common, &db->sysport_events_list[sysport], list) {
        if (gc_all || time_after(now, iter_common->since + PTP_GC_TIME)) {
            sx_clock_log_add_string(&priv->ptp.hwd.spc1.log_errors,
...
int sx_ptp_dev_init_spc1(struct sx_priv *priv, ptp_mode_t ptp_mode)
{
    ...
    err = sx_clock_log_init(&priv->ptp.hwd.spc1.log_errors, 1000);
    if (err) {
...
polling_thread_failed:
    sx_clock_log_deinit(&priv->ptp.hwd.spc1.log_errors);
log_init_failed:
    __delete_caches();
...
int sx_ptp_dump_spc1(struct sx_priv *priv, struct seq_file *m, void *v)
{
    unsigned long now = jiffies;
    ...
    sx_clock_log_dump(&priv->ptp.hwd.spc1.log_errors, m, "PTP error log");
