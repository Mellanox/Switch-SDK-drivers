...
static bool __dev_iterator_trap_reg_dump(struct sx_dev *dev, void *context)
{
    ...
    snprintf(header, HEADER_MAX_LEN, "Device Trap Registration - device_id %u", dev->device_id);
    print_header(dec->m, header);

    print_table_header(dec->m, "%-30s | %-8s | %-4s | %-21s | %-12s | %-25s | %-15s | %s",
    ...
    for (synd = 0; synd < NUM_HW_SYNDROMES + 1; synd++) {
        if (!list_empty(&info->per_synd_list[synd])) {
            list_for_each(pos, &info->per_synd_list[synd]) {
                ...
                print_listener(dec->m, synd, listener);
            }
        }
    }
...
static bool __dev_iterator_trap_filter_reg_dump(struct sx_dev *dev, void *context)
{
    ...
    snprintf(header, HEADER_MAX_LEN, "Device Trap Registrations and Filters - device_id %u", dev->device_id);
    print_header(dec->m, header);
    ...
    for (synd = 0; synd < NUM_HW_SYNDROMES + 1; synd++) {
        if (!list_empty(&info->per_synd_list[synd])) {
            list_for_each(pos, &info->per_synd_list[synd]) {
                ...
                print_table_header(dec->m, "%s", "listener Reg/Filter info:");
...
static bool __dev_iterator_host_ifc_stats_dump(struct sx_dev *dev, void *context)
{
    ...
    snprintf(header, HEADER_MAX_LEN, "host-ifc stats dump - device_id %u", dev->device_id);
    print_header(dec->m, header);


    print_table_header(dec->m, "%-30s   %-15s",
                       "RX-type", "RX-packets");
    ...
    print_table_header(dec->m, "%-30s   %-15s   %-15s",
    ...
    print_table_header(dec->m, "%-4s   %-15s   %-15s",
    ...
    print_table_header(dec->m, "%-3s   %-15s   %-15s",
...
static bool __dev_iterator_unconsumed_pkt_dump(struct sx_dev *dev, void *context)
{
    ...
    snprintf(header, HEADER_MAX_LEN, "Unconsumed packets - device_id %u", dev->device_id);
    print_header(dec->m, header);

    print_table_header(dec->m, "%-40s|%-8s| %s",
...
static int __rif_data_dump(struct seq_file *m, void *v)
{
    struct sx_dev_db_iterator_seq_file_context dec = {
        .m = m,
        .v = v
...
static int __sdq_dump(struct seq_file *m, void *v)
{
    struct sx_dev_db_iterator_seq_file_context dec = {
        .m = m,
        .v = v
...
static bool __dev_iterator_trap_filter_dump(struct sx_dev *dev, void *context)
{
    ...
    snprintf(header, HEADER_MAX_LEN, "trap filter dump - device_id %u", dev->device_id);
    print_header(dec->m, header);

    print_table_header(dec->m, "%-40s   %-7s   %-7s",
    ...
    print_table_header(dec->m, "%-40s   %-7s   %-6s",
...
int __init sx_dbg_dump_proc_fs_init(void)
{
    ...
    sx_dbg_dump_proc_fs_register("rif_data_dump", __rif_data_dump, NULL);
    sx_dbg_dump_proc_fs_register("sdq_dump", __sdq_dump, NULL);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 7, 0))
    ...
    sx_dbg_dump_proc_fs_register("bulk_cntr_dump", bulk_cntr_db_dump, NULL);
    sx_dbg_dump_proc_fs_register("accuflow_dump", sx_af_counters_dump, sx_af_counters_dump_size);
    sx_dbg_dump_proc_fs_register("dev_db_dump", sx_dev_db_dump, NULL);
    sx_dbg_dump_proc_fs_register("health_check_dump", sx_health_check_dump, NULL);
