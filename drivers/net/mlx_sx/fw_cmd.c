...
static void sx_fw_2_ku_query_fw(struct sx_priv *priv, const struct sx_fw *fw, struct ku_query_fw *query_fw)
{
    ...
    query_fw->fw_cap_ind_mod = !!SX_IS_FW_CAP(priv, SX_FW_CAP_IND_MOD);
...
#define QUERY_FW_IN_MB_INFO_OFFSET  0x04
    ...
    printk(KERN_INFO "HCR%d (Dev %d): In-Mailbox [offset=0x%x, size=0x%x], Out-Mailbox [offset=0x%x, size=0x%x]\n",
           hcr_number + 1,
           sx_dev_id,
...
int sx_SET_PROFILE(struct sx_dev *dev, struct ku_profile *profile, struct profile_driver_params *params)
{
    ...
    /* if SDK is not using independent-module, need to trigger hw-mgmt here */
    if (!err && (profile->set_mask_0_63 & SX_INDEPENDENT_MODULE_MASK) && !profile->module_master_fw_default) {
