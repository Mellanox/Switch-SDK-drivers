...
/* this is used to keep the long_cmd timeout already configured in FW */
#define LONG_COMMAND_TIMEOUT_DEFAULT_VALUE ((u8) - 1)
                        ...
                        sxd_event_health_data_t   *event_data)
{
    ...
    printk(KERN_NOTICE "Health check: new failure (dev=%u, severity='%s', cause=%d ['%s'], irisc=%u, desc='%s')\n",
           info->dev->device_id,
    ...
    if (__is_qtm_device(info->dev) && (cause != SXD_HEALTH_CAUSE_FW_LONG_COMMAND) &&
        ...
        printk(KERN_NOTICE "Health check: event is not sent due to long command that was detected!\n");
        info->long_command_detected = true;
        return;
    }
    ...
    for (sdq_id = 0; sdq_id < max_sdq; sdq_id++) {
        ...
        if (sdq) {
            ...
            if (skb_headroom(skb) < ISX_HDR_SIZE) {
                ...
                if (!new_skb) {
                    printk(KERN_ERR "Health check: failed to allocate SKB to test SDQ\n");
            ...
            if (ret != 0) {
                printk(KERN_ERR "Health check: failed to send SKB to test SDQ [sdq=%d, err=%d]\n", sdq_id, ret);
                goto out;
            }
        }
    }
...
static int __sx_health_add_sdk_thread_failure(u8 dev_id, const sdk_threads_info_t* threads_info)
{
    ...
    mutex_lock(&info->lock);
    snprintf(err_msg, sizeof(err_msg) - 1, "Thread %d [%s]", threads_info->sdk_thread_id, threads_info->error_msg);
    __add_issue(info, threads_info->severity, SXD_HEALTH_CAUSE_SDK_THREADS, DBG_ALL_IRISCS, err_msg, NULL);
    mutex_unlock(&info->lock);
    return 0;
}
...
static void __sx_health_check_cycle(struct work_struct *work)
{
    ...
    if (__is_qtm_device(info->dev) && __is_long_command_detected(info)) {
        ...
        if (info->long_command_event_counter >= LONG_COMMAND_MAX_COUNTER) {
            ...
            printk(KERN_ERR "Health check: long command was not released after %d seconds, "
...
issu_supported_features:
    ...
    if (info->fatal_cause != SXD_HEALTH_CAUSE_NONE) {
        ...
        printk(KERN_ERR "SXD_HEALTH_FATAL: device=%u, cause=%d ['%s'] - stopping further device monitoring!\n",
               info->dev->device_id,
               info->fatal_cause,
    ...
    if (info->major_issues_detected >= info->config.failures_num) {
        ...
        printk(KERN_ERR "SXD_HEALTH_FATAL: device %u is set to FATAL because of too many major issues - "
               "stopping further device monitoring!\n",
               info->dev->device_id);
        goto out;
    }
                                 ...
                                 struct ku_mfgd_reg            *active_mfgd)
{
    ...
    /* if long_cmd_timeout_sec == LONG_COMMAND_TIMEOUT_DEFAULT_VALUE, we keep the current config. otherwise, set long_cmd_timeout_value */
    if (long_cmd_timeout_sec != LONG_COMMAND_TIMEOUT_DEFAULT_VALUE) {
        reg_mfgd.mfgd_reg.long_cmd_timeout_value = long_cmd_timeout_sec;
        if (long_cmd_timeout_sec == 0) {
            printk(KERN_NOTICE "Health-Check [FW long-command]: disabling feature\n");
        } else {
            printk(KERN_NOTICE "Health-Check [FW long-command]: setting timeout to %u seconds\n",
                   long_cmd_timeout_sec);
        }
    } else {
        printk(KERN_NOTICE "Health-Check [FW long-command]: keeping FW value (%u seconds)\n",
                        ...
                        bool                        fatal_error_mode_active)
{
    ...
    u8                         mfgd_long_cmd_timeout = LONG_COMMAND_TIMEOUT_DEFAULT_VALUE;
    ...
    case SXD_CHIP_TYPE_SPECTRUM4:
        ...
        mfgd_long_cmd_timeout = 0; /* disable long_cmd feature! */
        break;
...
static void __info_dealloc(struct sx_health_dev_info *info)
{
    ...
    err = __sx_health_send_mfgd(info->dev,
                                ...
                                SXD_MFGD_FW_FATAL_EVENT_TEST_DONT_TEST_E,
                                LONG_COMMAND_TIMEOUT_DEFAULT_VALUE,
                                NULL);
    if (err) {
        printk(KERN_INFO "Health-Check: failed to disable monitoring (MFGD) on device %d [err=%d]\n",
...
static int __health_check_dbg_cmd_handler(int argc, const char *argv[], void *context)
{
    ...
    if (argc != 3) {
        printk(KERN_NOTICE "Health check: debug command - invalid number of arguments\n");
        return -EINVAL;
    }

    err = kstrtoint(argv[1], 10, &dev_id);
    if (err) {
        printk(KERN_NOTICE "Health check: debug command - device ID is invalid\n");
        return -EINVAL;
    }
    ...
    info = __info_find(dev_id);
    if (!info) {
        printk(KERN_NOTICE "Health check: debug command - device not found\n");
        err = -ENODEV;
        goto out;
    }

    if (strcmp(argv[2], "stop") == 0) {
        printk(KERN_NOTICE "Health check: debug command - stopping health-check for device %d\n", dev_id);
        __info_dealloc(info);
        goto out;
    }
    ...
    if (strcmp(argv[2], "mute") == 0) {
        printk(KERN_NOTICE "Health check: debug command - muting health events for device %d\n", dev_id);
        info->config.debug_state = HEALTH_DEBUG_MUTE;
    } else if (strcmp(argv[2], "normal") == 0) {
        printk(KERN_NOTICE "Health check: debug command - normal health-check functionality for device %d\n", dev_id);
        info->config.debug_state = HEALTH_DEBUG_NORMAL;
    } else {
        printk(KERN_NOTICE "Health check: debug command - invalid argument [%s]\n", argv[2]);
        err = -EINVAL;
...
int sx_health_check_init(void)
{
    ...
    if (err) {
        printk(KERN_ERR "Health check: failed to create sysfs entry (err=%d)\n", err);
        goto out;
    }

    err = sx_dbg_dump_write_handler_register("health_check",
                                             "<dev_id> <mute | stop | normal>",
                                             "health check debug commands",
                                             __health_check_dbg_cmd_handler,
                                             NULL);
    if (err) {
        printk(KERN_WARNING "Health check: failed to register health-check debug handler (not mandatory) (err=%d)\n",
               err);
        err = 0;
    }
...
static void __mfde_work(struct work_struct *work)
{
    ...
    case SXD_MFDE_EVENT_ID_CRSPACE_TIMEOUT_E:
        ...
        if (err) {
            printk(KERN_ERR "SXD_HEALTH_FATAL: Health check: failed to get kernel crspace timeout print err =%d\n",
                   err);
            kfree(ecw);
            goto out;
        }
        ...
        printk(KERN_ERR "SXD_HEALTH_FATAL: %s", error_msg);
        break;
    ...
    default:
        printk(KERN_ERR "SXD_HEALTH_FATAL: Health check: the given event id =%d is out of range", mfde_reg->event_id);
        kfree(ecw);
        goto out;
    }
