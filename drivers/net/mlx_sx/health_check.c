                                        ...
                                        struct sx_dev *dev)
{
    ...
    chip_type = priv->dev_info.chip_type;
                        ...
                        sxd_event_health_data_t     *event_data)
{
    ...
    if ((info->config.min_severity >= issue_severity) && (issue_severity != SXD_HEALTH_SEVERITY_FATAL)) {
        ...
        printk(KERN_WARNING "Health check issue found: device=%d, cause=%s cause id=%d, severity=%s irisc_id=0x%x\n",
               info->dev->device_id,
...
static void __mfde_work(struct work_struct *work)
{
    ...
    if (!info) {
        printk(KERN_NOTICE "Health check Disabled: got MDFE event but will not monitor!!!\n");
        mutex_unlock(&__health_check_lock);
        goto out;
    }
    mutex_unlock(&__health_check_lock);
