                                        ...
                                        struct sx_dev *dev)
{
    ...
    chip_type = priv->dev.pdev->device;
...
void sx_health_check_mfde_error_update(struct ku_mfde_reg *mfde_reg,  struct sx_dev * dev)
{
    ...
    uint64_t                          log_ip = 0;
    char                            * assert_string;
    char                              print_message[FW_SOS_LOG_MAX_SIZE];
    sxd_event_health_notification_t   sdk_health;
    sxd_health_external_report_data_t event_ext_data;
    char                              error_msg[HEALTH_CHECK_EVENT_MSG_MAX];
    ...
    case SXD_MFDE_EVENT_ID_CRSPACE_TIMEOUT_E:
        ...
        snprintf(error_msg,
                 sizeof(error_msg),
                 "SXD_HEALTH_FW_FATAL: FW CR timeout on log_address:[0x%x] old_event:[%d] log_id:[%d] log_ip:[0x%llx] [%s]\n",
    ...
    case SXD_MFDE_EVENT_ID_KVD_IM_STOP_E:
        snprintf(error_msg, sizeof(error_msg), "SXD_HEALTH_FW_FATAL: FW KVM stopped, old_event:[%d] pipe mask:[%d] \n",
    ...
    case SXD_MFDE_EVENT_ID_FW_ASSERT_E:
        ...
        snprintf(error_msg,
                 sizeof(error_msg),
                 "SXD_HEALTH_FW_FATAL: FW assert msg: [%s], FW assert event - assert id) = [0x%x], assert_var0 = [0x%x], assert_var1 = [0x%x], assert_var2 = [0x%x],"
    ...
    case SXD_MFDE_EVENT_ID_FATAL_CAUSE_E:
        ...
        if (true == mfde_reg->event_params.fatal_cause.tile_v) {
            ...
            if (mfde_reg->event_params.fatal_cause.fw_cause) {
                snprintf(error_msg,
                         sizeof(error_msg),
                         "SXD_HEALTH_FW_FATAL:FW fatal cause: FW cause id = [0x%x], tile_index = [0x%x] \n",
                         mfde_reg->event_params.fatal_cause.cause_id,
                         tile);
            ...
            if (mfde_reg->event_params.fatal_cause.fw_cause) {
                snprintf(error_msg, sizeof(error_msg), "SXD_HEALTH_FW_FATAL:FW fatal cause: FW cause id = [0x%x] \n",
                         mfde_reg->event_params.fatal_cause.cause_id);
                if (mfde_reg->event_params.fatal_cause.test == FW_SOS_TEST) {
                    printk(KERN_WARNING " %s", error_msg);
                } else {
                        ...
                        sxd_event_health_data_t     *event_data)
{
    ...
    if ((info->config.min_severity >= issue_severity) && (issue_severity != SXD_HEALTH_SEVERITY_FATAL)) {
        sx_send_health_event(info->dev->device_id, cause, issue_severity, event_data, NULL);
        printk(KERN_WARNING "Health check issue found: device=%d, cause=%d, severity=%s\n",
               info->dev->device_id,
               cause,
                                       ...
                                       union sdk_health_cause_data *cause_data)
{
    ...
    list_for_each_entry(info, &__dev_info_list, dev_list) {
        ...
        __add_issue(info, issue_severity, cause, cause_data, NULL);
...
static void __check_catas(struct sx_health_dev_info *info)
{
    ...
    if (catas_data.catas_data.catas_buf & 0x2) {
        __add_issue(info, SXD_HEALTH_SEVERITY_FATAL, SXD_HEALTH_CAUSE_CATAS, &catas_data, NULL);
    }
}
...
static int __start_and_check_cmd_ifc(struct sx_health_dev_info *info)
{
    ...
    if (priv->debug_trigger_state.cmd_ifc) {
        __add_issue(info, SXD_HEALTH_SEVERITY_WARN, SXD_HEALTH_CAUSE_GO_BIT, &cause_data, NULL);
        goto out;
    }
...
static void __check_and_report_sdq_failure(struct sx_health_dev_info *info, u32 sub_bitmap_res)
{
    ...
    if (info->config.sdq.num_of_check_iter >= info->config.sdq.max_iter_allowed) {
        ...
        printk(KERN_ERR PFX "Health check:Did not receive completion for SDQ dqn (%u) \n",
               sub_bitmap_res);
        __add_issue(info, SXD_HEALTH_SEVERITY_WARN, SXD_HEALTH_CAUSE_SDQ, &dq_data, NULL);
...
static void __start_rdq_for_next_cycle(struct sx_health_dev_info *info)
{
    ...
    if (err) {
        ...
        __add_issue(info, SXD_HEALTH_SEVERITY_FATAL, SXD_HEALTH_CAUSE_FW_HETT, NULL, NULL);
    }
}
...
static void __check_rdq_from_last_cycle(struct sx_health_dev_info *info)
{
    ...
    if (sub_bitmap_res != ((u32) - 1)) {
        dq_data.dq_data.dqn = sub_bitmap_res;
        __add_issue(info, SXD_HEALTH_SEVERITY_WARN, SXD_HEALTH_CAUSE_RDQ, &dq_data, NULL);
    }
...
static int __sx_health_add_sdk_thread_failure(u8 dev_id, const sdk_threads_info_t* threads_info)
{
    ...
    __add_issue(info, threads_info->severity, SXD_HEALTH_CAUSE_SDK_THREADS, &cause_data, NULL);
...
static int __sx_health_add_sdk_emad_timeout_failure(u8 dev_id)
{
    ...
    mutex_lock(&info->lock);
    __add_issue(info, SXD_HEALTH_SEVERITY_FATAL, SXD_HEALTH_CAUSE_EMAD_TIMEOUT, &cause_data, NULL);
...
health_monitor:
    ...
    if (info->fatal_cause != SXD_HEALTH_CAUSE_NONE) {
        ...
        sx_send_health_event(info->dev->device_id,
                             ...
                             NULL);
        printk(KERN_ERR "SXD_HEALTH_FATAL:device %u found FATAL cause - not monitored anymore!\n",
               info->dev->device_id);
        ...
        sx_send_health_event(info->dev->device_id,
                             ...
                             NULL);
        printk(
            KERN_ERR "SXD_HEALTH_FATAL:device %u is set to FATAL because of too many major issues - not monitored anymore!\n",
            info->dev->device_id);
    } else {
                        ...
                        bool                        fatal_error_mode_active)
{
    ...
    switch (priv->dev_info.chip_type) {
    case SXD_CHIP_TYPE_SPECTRUM:
    case SXD_CHIP_TYPE_SPECTRUM2:
    case SXD_CHIP_TYPE_SPECTRUM3:
    case SXD_CHIP_TYPE_SPECTRUM4:
...
{
    bool new_ecc_update = false;

    if (ecw->data.ecc_stats.ecc_corrected != info->ecc_stats.ecc_corrected) {
    ...
    if (ecw->data.ecc_stats.ecc_uncorrected != info->ecc_stats.ecc_uncorrected) {
    ...
    if (ecw->dev_id == DEV_ID_ALL) {
        ...
        if (info) {
            ...
            if (__check_external_report(info, ecw)) {
                __add_issue(info, ecw->issue_severity, ecw->issue_cause, &ecw->cause_data,
...
void sx_health_external_report_all(u32 issue_severity, u32 issue_cause, const char *error_msg)
{
    ...
    sx_health_external_report(DEV_ID_ALL, issue_severity, issue_cause, &event_ext_data, error_msg);
}
...
void sx_health_handle_new_listener(struct sx_dev *dev, struct listener_entry *listener)
{
    ...
    if (info) {
        ...
        list_for_each_entry(failure, &info->issues_list, list) {
            ...
            if (failure->severity >= info->config.min_severity) {
                sx_send_health_event(dev->device_id, failure->cause, failure->severity, &failure->event_data,
                                     listener);
            }
        }
