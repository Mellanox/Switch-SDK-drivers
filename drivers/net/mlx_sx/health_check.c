...
static void __check_and_report_sdq_failure(struct sx_health_dev_info *info, u32 bit_map_compare_res)
{
    union sdk_health_cause_data dq_data;

    /*failure - exceed the time frame  */
    if (info->config.sdq.num_of_check_iter >= info->config.sdq.max_iter_allowed) {
        dq_data.dq_data.dqn = bit_map_compare_res;
...
{
    struct sx_priv *priv = sx_priv(info->dev);
    u32             bit_map_compare_res = 0;

    info->config.sdq.num_of_check_iter++;

    bit_map_compare_res = __compare_bitmaps(&priv->operational_state.sdq_bitmap, &info->config.sdq.sdq_bitmap);
    if (bit_map_compare_res != ((u32) - 1)) {
        __check_and_report_sdq_failure(info, bit_map_compare_res);
    } else {
        info->config.sdq.num_of_check_iter = 0;
    }
}
