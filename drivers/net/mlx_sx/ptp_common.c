...
int sx_core_ptp_dev_init(struct sx_priv *priv, ptp_mode_t ptp_mode)
{
    ...
    SX_CLOCK_ACTIVITY_LOG(priv, "PTP initialized");
...
int sx_core_ptp_dev_cleanup(struct sx_priv *priv)
{
    ...
    SX_CLOCK_ACTIVITY_LOG(priv, "PTP cleanup");
...
static bool __dev_iterator(struct sx_dev *dev, void *context)
{
    struct sx_priv                             *priv = sx_priv(dev);
    struct sx_dev_db_iterator_seq_file_context *dec = (struct sx_dev_db_iterator_seq_file_context*)context;

    seq_printf(dec->m, "PTP DUMP (device #%u)\n", priv->dev.device_id);
    seq_printf(dec->m, "-----------------------\n");

    switch (sx_ptp_get_mode(priv)) {
    case KU_PTP_MODE_DISABLED:
        seq_printf(dec->m, "PTP mode: Disabled\n");
        goto out;

    case KU_PTP_MODE_EVENTS:
        seq_printf(dec->m, "PTP mode: Events\n");
        break;

    case KU_PTP_MODE_POLLING:
        seq_printf(dec->m, "PTP mode: Polling\n");
        break;

    default:
        seq_printf(dec->m, "PTP mode: N/A\n");
        goto out;
    }

    seq_printf(dec->m, "PTP CQn: %d\n\n", priv->ptp.ptp_cqn);

    seq_printf(dec->m, "%-40s   %-15s   %-15s\n", "Counter", "RX", "TX");
    seq_printf(dec->m, "-----------------------------------------------------------------------------\n");
    ...
    seq_printf(dec->m, "%-40s   %-15llu   %-15llu\n",
               "Timestamp required",
    ...
    SX_CLOCK_DEV_SPECIFIC_CB(dev, sx_ptp_dump, priv, dec->m, dec->v);
...
int sx_dbg_ptp_dump_proc_show(struct seq_file *m, void *v)
{
    ...
    sx_dev_db_iterate(__dev_iterator, &dec);
    return 0;
}
