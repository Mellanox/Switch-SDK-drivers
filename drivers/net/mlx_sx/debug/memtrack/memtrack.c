    ...
    if ((memtype != MEMTRACK_DMA_MAP_SINGLE) && (memtype != MEMTRACK_DMA_MAP_PAGE) &&
        ...
        while (cur_mem_info_p != NULL) {
            if ((cur_mem_info_p->addr == addr) && (cur_mem_info_p->dev == dev)) {
                ...
                printk(
                    KERN_ERR "mtl rsc inconsistency: %s: %s::%lu: %s @ addr=0x%lX which is already known from %s:%lu\n",
                    __func__,
                    filename,
                    line_num,
        ...
        if ((cur_mem_info_p->addr == addr) && (cur_mem_info_p->dev == dev)) {
            ...
            if ((memtype == MEMTRACK_DMA_MAP_SINGLE) || (memtype == MEMTRACK_DMA_MAP_PAGE) ||
                ...
                if (direction != cur_mem_info_p->direction) {
                    printk(
                        KERN_ERR "mtl rsc inconsistency: %s: %s::%lu: %s bad direction for addr 0x%lX: alloc:0x%x, free:0x%x (allocated in %s::%lu)\n",
                        __func__,
                        filename,
                        line_num,
                ...
                if (size != cur_mem_info_p->size) {
                    printk(
                        KERN_ERR "mtl rsc inconsistency: %s: %s::%lu: %s bad size for addr 0x%lX: size:%lu, free:%lu (allocated in %s::%lu)\n",
                        __func__,
                        filename,
                        line_num,
                        memtype_free_str(memtype),
    ...
    while (cur_mem_info_p != NULL) {
        if (cur_mem_info_p->addr == addr) {
            ...
            if (cur_mem_info_p->size != size) {
                printk(
                    KERN_ERR "mtl size inconsistency: %s: %s::%lu: try to %s at address=0x%lX with size %lu while was created with size %lu\n",
                    __func__,
                    filename,
                    line_num,
                    memtype_free_str(memtype),
