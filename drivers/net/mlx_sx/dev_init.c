...
static int __sx_init_chip_callbacks(struct sx_dev *dev)
{
    ...
    memcpy(&sx_priv(dev)->dev_info.mgir, &reg_data.mgir_reg, sizeof(struct ku_mgir_reg));
...
static bool __check_secure_fw_boot_status(struct pci_dev *pdev, struct sx_priv **priv)
{
    struct sx_priv                  *dummy_priv = NULL;
    const char                      *busname = dev_name(&pdev->dev);
    unsigned short                   pci_dev_id = pdev->device;
    enum sxd_secure_fw_boot_status_e secure_fw_boot_status = SXD_SECURE_FW_BOOT_STATUS_OK_E;
    int                              chip_type = 0;
    ...
    case SPECTRUM4_FLASH_MODE_PCI_DEV_ID:
        ...
        chip_type = SXD_CHIP_TYPE_SPECTRUM4;
        secure_fw_boot_status = SXD_SECURE_FW_BOOT_STATUS_SPC4_IN_FLASH_RECOVERY_E;
        printk(KERN_ERR "Secure-FW-Status: Spectrum-4 on bus %s is in 'Flash Mode' status\n", busname);
        break;

    case SPECTRUM4_RMA_PCI_DEV_ID:
        chip_info_type = SPECTRUM4_PCI_DEV_ID;
        chip_type = SXD_CHIP_TYPE_SPECTRUM4;
        secure_fw_boot_status = SXD_SECURE_FW_BOOT_STATUS_SPC4_RMA_E;
        printk(KERN_ERR "Secure-FW-Status: Spectrum-4 on bus %s is in 'RMA' status\n", busname);
        break;

    default:
        return true; /* everything's fine with this device, no fw-secure-boot issue */
    }

    dummy_priv = __create_dummy_device(chip_type, pdev);
    if (dummy_priv) {
        dummy_priv->dev.pdev = NULL;
        dummy_priv->secure_fw_boot_status = secure_fw_boot_status;
    }
                              ...
                              struct pci_restart_saved_params *saved_params)
{
    ...
    if (!__check_secure_fw_boot_status(pdev, &priv)) {
        if (!priv) {
            err = -ENODEV;
        } else {
            __probe_params.secure_fw_boot_failures++;
        }
    ...
    if (__probe_params.do_reset) {
        ...
        sx_send_udev_event(pdev, priv, KOBJ_ADD);
    }
...
static void __pci_remove_common(struct pci_dev *pdev, bool keep_listeners)
{
    ...
    if (sx_core_secure_fw_is_faulty(dev)) {
    ...
    if (priv->kobj_add_done) {
        ...
        sx_send_udev_event(pdev, priv, KOBJ_REMOVE);
    }
                                 ...
                                 u32                driver_flag)
{
    ...
    if (__probe_params.total_probes != __probe_params.successful_probes + __probe_params.secure_fw_boot_failures) {
...
void sx_send_udev_event(struct pci_dev *pdev, struct sx_priv *priv, enum kobject_action action)
{
    ...
    printk(KERN_NOTICE "sending UDEV event from sx-core driver [bus=%s, action=%d (%s)]\n",
           dev_name(&pdev->dev), action, action_str);
...
int sx_restart_one_pci(struct sx_dev *dev)
{
    ...
    saved_params.module_sysfs_arr = sx_priv(dev)->module_sysfs_arr; /* save module eeprom sysfs info */
...
int sx_core_init_one(struct sx_priv **sx_priv, struct pci_restart_saved_params *saved_params)
{
    ...
    priv->secure_fw_boot_status = SXD_SECURE_FW_BOOT_STATUS_OK_E;
...
bool sx_core_secure_fw_is_faulty(struct sx_dev *dev)
{
    return (sx_priv(dev)->secure_fw_boot_status != SXD_SECURE_FW_BOOT_STATUS_OK_E);
}
