...
static int __sx_init_chip_callbacks(struct sx_dev *dev)
{
    ...
    if (!ok) {
        printk(KERN_ERR "Failed to get chip information (err=%d)\n", err);
        goto out;
    }
                              ...
                              struct pci_restart_saved_params *saved_params)
{
    ...
    dev_notice(&pdev->dev, "Probe device %u (0x%x)\n", pdev->device, pdev->device);
    ...
    if (!__check_fw_boot_status(pdev, &priv)) {
        ...
        if (err) {
            dev_err(&pdev->dev, "failed to get device ID from PCI-device (err=%d)\n", err);
        } else {
    ...
    if (err) {
        dev_err(&pdev->dev, "failed to initialize device private data (err=%d)\n", err);
        goto out;
    }
    ...
    if (!(pci_resource_flags(pdev, 0) & IORESOURCE_MEM) ||
        ...
        dev_err(&pdev->dev, "missing BAR0 or invalid size [flags=0x%lx, len=0x%llx]\n",
    ...
    if (err) {
        dev_err(&pdev->dev, "failed to request control region (err=%d)\n", err);
    ...
    if (err) {
        dev_warn(&pdev->dev, "failed to set DMA mask to 64-bit, fallback to 32-bit (err=%d)\n", err);
        err = dma_set_mask_and_coherent(&pdev->dev, DMA_BIT_MASK(32));
        if (err) {
            dev_err(&pdev->dev, "failed to set DMA mask to 32-bit (err=%d)\n", err);
    ...
    if (err) {
        dev_err(&pdev->dev, "failed to add device to database (err=%d)\n", err);
    ...
    if (saved_params) { /* PCI_RESTART flow */
        ...
        if (err) {
            dev_err(&pdev->dev, "failed to add character-device (err=%d)\n", err);
    ...
    if (err) {
        dev_err(&pdev->dev, "failed to initialize DPT device attributes to PCI (err=%d)\n", err);
    ...
    if (sx_cmd_pool_create(dev)) {
        dev_err(&pdev->dev, "failed to create command buffer pool (err=%d)\n", err);
    ...
    if (sx_cmd_init_pci(dev)) {
        dev_err(&pdev->dev, "failed to initialize command interface (err=%d)\n", err);
        goto prepare_dev_for_cr_dump;
    }
    ...
    if (skip_reset && !sx_core_has_predefined_devices()) {
        dev_notice(&pdev->dev, "in SKIP_RESET mode, reassigning device ID from 255 to %d\n",
                   SX_SINGLE_ASIC_DEFAULT_DEV_ID);
        err = sx_dev_db_reassign_dev_id_to_default_device(SX_SINGLE_ASIC_DEFAULT_DEV_ID);
        if (err) {
            dev_err(&pdev->dev, "failed to reassign dev_id from 255 to %d internally (err=%d)\n",
    ...
    if (err) {
        dev_err(&pdev->dev, "failed to initialize chip-type callbacks (err=%d)\n", err);
        goto prepare_dev_for_cr_dump;
    }
    ...
    dev_notice(&pdev->dev, "performing SW reset is SKIPPED in PD mode\n");
#else
    ...
    if (err) {
        dev_err(&pdev->dev, "failed to reset ASIC (err=%d)\n", err);
        goto prepare_dev_for_cr_dump;
    }
#endif
    ...
    if (err) {
        dev_err(&pdev->dev, "failed to initialize board (err=%d)\n", err);
        goto prepare_dev_for_cr_dump;
    }
    ...
    if (err) {
        dev_err(&pdev->dev, "failed to map doorbell area (err=%d)\n", err);
        goto prepare_dev_for_cr_dump;
    }
    ...
    if (err) {
        dev_err(&pdev->dev, "failed to map CR-Space area (err=%d)\n", err);
        goto prepare_dev_for_cr_dump;
    }
    ...
    if (err) {
        dev_err(&pdev->dev, "failed to setup MSI-X interface (err=%d)\n", err);
        goto prepare_dev_for_cr_dump;
    }
    ...
    if (err) {
        dev_err(&pdev->dev, "failed to initialize clock (err=%d)\n", err);
        goto prepare_dev_for_cr_dump;
    }
    ...
    if (err) {
        dev_err(&pdev->dev, "failed to initialize CR-Space dump parameters (err=%d)\n", err);
        goto prepare_dev_for_cr_dump;
    }
    ...
    if (!__probe_params.is_oob) {
        ...
        if (err) {
            dev_err(&pdev->dev, "failed to create ASIC sysfs (err=%d)\n", err);
            goto prepare_dev_for_cr_dump;
        }
        ...
        if (err) {
            dev_err(&pdev->dev, "failed to initialize temperature monitoring (err=%d)\n", err);
            goto prepare_dev_for_cr_dump;
        }

        err = sx_asic_perf_counter_init(dev);
        if (err) {
            dev_err(&pdev->dev, "failed to initialize performance counters (err=%d)\n", err);
            goto prepare_dev_for_cr_dump;
        }
    ...
    dev_notice(&pdev->dev, "PCI probe completed successfully\n");
    return 0;

prepare_dev_for_cr_dump:
    dev_err(&pdev->dev, "device is in FATAL state during probe! preparing it for CR-Space dump\n");
    ...
    if (!SX_PROBE_STEP_CHECK(priv, SX_PROBE_STEP_MAP_CR_SPACE)) {
        ...
        if (err) {
             ...
             */
            dev_err(&pdev->dev, "failed to prepare device for CR-Space dump (err=%d)\n", err);
        } else {
            SX_PROBE_STEP_SET(priv, SX_PROBE_STEP_MAP_CR_SPACE); /* successful __sx_map_cr_space_area() */
        }
    }
