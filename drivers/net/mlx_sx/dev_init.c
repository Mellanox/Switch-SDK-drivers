...
static int __pci_probe_common(struct pci_dev               *pdev,
                              ...
                              struct listeners_and_rf_info *orig_listeners);
                              ...
                              struct listeners_and_rf_info *orig_listeners)
{
    ...
    printk(KERN_INFO PFX "Probe %s(%d) device %u\n", __FUNCTION__, __LINE__, \
           pdev->device);
    err = sx_core_init_one(&priv, orig_listeners);
    if (err) {
...
int sx_restart_one_pci(struct sx_dev *dev, bool do_reset, bool keep_listeners)
{
    ...
    struct kobject               *orig_kobj[SX_KOBJECT_MAX + 1];
    struct module_sysfs_node     *orig_module_eeprom_arr = NULL;
    int                           err = 0;
    ...
    /* save sysfs info */
    memcpy(orig_kobj, sx_priv(dev)->kobj, sizeof(orig_kobj));

    /* save module eeprom sysfs info */
    orig_module_eeprom_arr = sx_priv(dev)->module_eeprom_arr;

    __pci_remove_common(pdev, keep_listeners);
    err = __pci_probe_common(pdev, NULL, &new_dev, orig_listeners);
    if (err) {
    ...
    /* restore sysfs info */
    memcpy(sx_priv(new_dev)->kobj, orig_kobj, sizeof(sx_priv(new_dev)->kobj));

    /* restore module eeprom sysfs info */
    sx_priv(new_dev)->module_eeprom_arr = orig_module_eeprom_arr;
...
int sx_core_init_one(struct sx_priv **sx_priv, struct listeners_and_rf_info *orig_listeners)
{
    struct sx_priv *priv;
    struct sx_dev  *dev;
    int             i, j, err;
    ...
    if (orig_listeners) {
        priv->listeners_and_rf_db.info = orig_listeners;
    } else {
