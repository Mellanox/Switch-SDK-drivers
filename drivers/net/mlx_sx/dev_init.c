...
static int __pci_probe_common(struct pci_dev *pdev, const struct pci_device_id *id, struct sx_dev **new_dev);
static void __pci_remove_common(struct pci_dev *pdev);
...
static void __pci_remove_common(struct pci_dev *pdev)
{
    ...
    sx_health_check_dev_deinit(dev); /* MUST be called before sx_dev_db_remove_device() */
...
static int __pci_probe_core(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    return __pci_probe_common(pdev, id, NULL);
}

static void __pci_remove_core(struct pci_dev *pdev)
{
    printk(KERN_NOTICE "remove a PCI core device\n");
    __pci_remove_common(pdev);
...
static int __pci_probe_oob(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    return __pci_probe_common(pdev, id, &sx_glb.oob_backbone_dev);
}
...
#ifdef NO_PCI
static void __sx_destroy_sx(struct sx_dev *dev)
{
    sx_af_counters_deinit();
...
int sx_restart_one_pci(struct sx_dev *dev, bool do_reset, bool keep_listeners)
{
    struct sx_dev                *new_dev = NULL;
    struct pci_dev               *pdev = NULL;
    struct listeners_and_rf_info *orig_listeners = NULL;
    int                           err = 0;
    ...
    if (keep_listeners) {
        printk(KERN_INFO "keeping old device's listeners for the new device\n");

        /* save listeners so we add them back to the new device */
        orig_listeners = sx_priv(dev)->listeners_and_rf_db.info;

        /* must assign NULL on the listeners so sx_core_remove_one() will not deallocate them */
        sx_priv(dev)->listeners_and_rf_db.info = NULL;
    }

    sx_core_dispatch_event(dev, SX_DEV_EVENT_PCI_PRE_RESTART, NULL);
    __pci_remove_common(pdev);
    err = __pci_probe_common(pdev, NULL, &new_dev);

    if (err) {
        sx_core_dispatch_event(NULL, SX_DEV_EVENT_PCI_POST_RESTART, NULL);
    } else {
        if (keep_listeners) {
            /* reassign previous listeners database to the new device, but first remove
             * the database allocated by sx_core_init_one() */
            kfree(sx_priv(new_dev)->listeners_and_rf_db.info);
            sx_priv(new_dev)->listeners_and_rf_db.info = orig_listeners;
        }

        sx_core_dispatch_event(new_dev, SX_DEV_EVENT_PCI_POST_RESTART, NULL);
    }
 ...
 * 1. on a VM machine (NO_PCI is defined)
 * 2. when a problem occurs and there is no PCI device detected on the bus.
 * 3. on OOB system.
 */
