...
static bool __check_fw_boot_status(struct pci_dev *pdev, struct sx_priv **priv)
{
    ...
    if (dummy_priv) {
        ...
        dummy_priv->dev_info.chip_type = chip_type;
        dummy_priv->dev_info.fw_boot_status = fw_boot_status;
    }
...
{
    return (sx_priv(dev)->dev_info.fw_boot_status != SXD_FW_BOOT_STATUS_OK_E);
}
