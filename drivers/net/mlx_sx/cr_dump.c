...
static int __fw_crdump_precheck(struct sx_dev *dev, dma_addr_t dma_addr, u8 opcode, int dump_size)
{
    ...
    case SX_CR_DUMP_OP_START_GDB:
        __dumped_bytes = 0;
        cap_dump_host_size = __cap_dump_host_size_gdb;
        break;
...
static int __fw_crdump_postcheck(struct sx_dev *dev, u8 opcode, unsigned char *buf, struct sx_cr_dump_ret *cr_dump_ret)
{
    ...
    switch (opcode) {
    case SX_CR_DUMP_OP_START_FLAT:
    case SX_CR_DUMP_OP_START_FLAT_WAIT:
    case SX_CR_DUMP_OP_START_GW:
    case SX_CR_DUMP_OP_START_GDB:
...
{
    return (ret_opcode == SX_CR_DUMP_OP_START_FLAT_WAIT) || (ret_opcode == SX_CR_DUMP_OP_CONT_WAIT);
}
                                ...
                                struct sx_cr_dump_ret *cr_dump_ret)
{
    ...
    do {
        if (lay1_waited_num == 0) {
            cr_dump_wait_us = SX_CR_DUMP_MIN_WAIT_FW_DUMP_MS(__cr_dump_memblk_size) * 1000;
            ...
            cr_dump_wait_us = SX_CR_DUMP_MAX_WAIT_FW_DUMP_MS(__cr_dump_memblk_size) * 1000 - cr_dump_wait_us;
        } else {
...
static int __parse_cr_dump_params(struct ku_cr_dump *params, struct sx_dev **dev, u8 *opcode)
{
    ...
    err = sx_dpt_get_pcie_info(params->dev_id, &pcie_info);
    if (err) {
    ...
    *dev = (struct sx_dev*)pcie_info->sx_dev;
out:
    if (locked) {
