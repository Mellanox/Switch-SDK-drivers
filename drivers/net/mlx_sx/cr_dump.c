...
void sx_core_cr_dump_notify_dump_completion(struct sx_dev *dev, bool query, fw_dump_completion_state_t* state)
{
    ...
    if (query) {
        ...
        fw_dump_completion_state = (complete_dword & 0x03000000) >> 24;
        *state = fw_dump_completion_state;
        return;
    }

    /* Change only the completion state. */
    complete_dword &= 0xFCFFFFFF;
...
int sx_core_cr_dump_init(struct sx_priv *priv)
{
    ...
    printk(
        KERN_NOTICE "cr_dump init: cap_dump_host_size_flat:%u, cap_dump_host_size_reduced_flat:%u, cap_dump_host_size_gw:%u, cap_dump_host_size_gdb:%u.\n",
