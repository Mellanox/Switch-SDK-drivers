...
static int __ptp_pps_enable(struct ptp_clock_info *ptp, struct ptp_clock_request *rq, int on)
{
    ...
    /* check if pps capability is supported */
    if (!priv->hw_clock.clock_info.pps) {
        return -EOPNOTSUPP;
    }
...
static int __ptp_pps_perout_configure(struct sx_priv *priv, struct ptp_clock_request *rq, int on)
{
    ...
    pin = ptp_find_pin(priv->hw_clock.clock, PTP_PF_PEROUT, rq->perout.index);
    if (pin < 0) {
        return -EBUSY;
    }
    ...
    sx_cmd_set_op_tlv(&mtpps.op_tlv, MTPPS_REG_ID, 2); /* Write MTPPS */
...
static int __ptp_pps_extts_configure(struct sx_priv *priv, struct ptp_clock_request *rq, int on)
{
    ...
    pin = ptp_find_pin(priv->hw_clock.clock, PTP_PF_EXTTS, rq->extts.index);
    if (pin < 0) {
        return -EBUSY;
    }
    ...
    memset(&mtpps, 0, sizeof(mtpps));
    mtpps.dev_id = priv->dev.device_id;
    sx_cmd_set_op_tlv(&mtpps.op_tlv, MTPPS_REG_ID, 2); /* Set MTPPS */
...
static int __ptp_pps_capabilities_get(struct sx_priv *priv)
{
    ...
    memset(&mtpps, 0, sizeof(mtpps));
    mtpps.dev_id = priv->dev.device_id;
    sx_cmd_set_op_tlv(&mtpps.op_tlv, MTPPS_REG_ID, 1);  /* Query MTPPS */
    err = sx_ACCESS_REG_MTPPS(&priv->dev, &mtpps);
    if (err) {
...
int sx_clock_dev_init_spc2(struct sx_priv *priv)
{
    ...
    if (false) {
        /* Temporarily DON'T init PPS until FW support is completed */
        /* Init PPS: */
