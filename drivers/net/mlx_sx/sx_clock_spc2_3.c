...
static int __init_adjfine_params(struct sx_priv *priv)
{
    ...
    if (adjfine_ppm_supported) {
        ...
        printk(
            KERN_NOTICE "Frequency adjustment (scaled PPM unit) is supported, with log_max_freq_adjustment being %u.\n",
    ...
    printk(KERN_NOTICE "Frequency adjustment PPB log_max_freq_adjustment is %u.\n",
           log_max_freq_adjustment);
...
static int __fallback_to_adjfreq(struct ptp_clock_info *ptp, long ppb)
{
    ...
    pr_debug(PFX "__fallback_to_adjfreq: ppb %ld.\n", ppb);

    if (!IS_VALID_ADJFREQ_PPB_VALUE(priv, ppb)) {
        printk(
            KERN_NOTICE "scaled_ppm adjustment to ppb: input freq adjustment value %ld ppb is out of supported ppb range (%ld).\n",
            ppb,
...
static int __adjfine_spc2(struct ptp_clock_info *ptp, long scaled_ppm)
{
    ...
    pr_debug(PFX "%s: scaled_ppm %ld (= ppb %ld).\n", __func__, scaled_ppm, ppb);

    if (!priv->hw_clock.hwd.spc2.is_scaled_ppm_freq_adj_supported) {
        printk(
            KERN_NOTICE "ASIC does not support frequency adjustment with scaled_ppm unit, will try to fallback to use ppb unit.\n");
        try_fallback_to_ppb_unit = true;
    } else if (!IS_VALID_ADJFINE_PPB_VALUE(priv, ppb)) {
        printk(
            KERN_NOTICE "Input %ld ppb is out of supported range (%ld) supported by scaled_ppm unit, will try to fallback to use ppb unit.\n",
            ppb,
            priv->hw_clock.hwd.spc2.max_scaled_ppm_freq_adj_ppb);
        try_fallback_to_ppb_unit = true;
    } else if (!IS_VALID_ADJFINE_SCALED_PPM_VALUE(scaled_ppm)) {
        printk(
            KERN_NOTICE "The %ld scaled_ppm (calculated from input %ld ppb) is out of supported range (S32_MAX), will try to fallback to use ppb unit.\n",
            scaled_ppm,
            ppb);
        try_fallback_to_ppb_unit = true;
    }
...
int sx_clock_dev_init_spc2(struct sx_priv *priv)
{
    ...
    err = sx_ACCESS_REG_MCAM(&priv->dev, &reg_mcam);
    if (err) {
        printk(KERN_ERR "Failed to read MCAM (err=%d)\n", err);
        goto failed_mcam;
    }
    ...
    if (err) {
        printk(KERN_ERR "failed to query fine adjustment related params (err=%d)\n", err);
        goto failed_reg;
    }
#endif
