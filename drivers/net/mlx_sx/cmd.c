    ...
    if (out_param && out_param->buf) {
        if (op == SX_CMD_ACCESS_REG) {
            if (priv->dev_sw_rst_flow) {
                ...
                if (err) {
                    printk(KERN_ERR "Failed to read operation TLV from output mailbox after reset (err=%d)\n", err);
                    goto out;
                }
            } else {
                ...
                if (err) {
                    printk(KERN_ERR "Failed to read Operation-TLV and Reg-TLV header from output mailbox (err=%d)\n",
                           err);
                    goto out;
                }
                ...
                if (sxd_emad_tlv_type(reg_tlv) != TLV_TYPE_REG_E) {
                    printk(KERN_ERR "TLV type is not Reg-TLV (type = %u)\n", sxd_emad_tlv_type(reg_tlv));
                    err = -EINVAL;
                    goto out;
                }
                ...
                if (err) {
                    printk(KERN_ERR "Failed to read register from output mailbox (err=%d)\n", err);
                    goto out;
                }
            }
        } else {
            ...
            if (err) {
                printk(KERN_ERR "Failed to read command-interface output mailbox for op=0x%x (err=%d)\n", op, err);
                goto out;
            }
        }
    }
...
out:
    if (SXD_HEALTH_CAUSE_NONE != cause) {
        ...
        if (sdk_health_test_and_disable(sx_dev_id)) {
            sx_send_health_event(sx_dev_id, cause, SXD_HEALTH_SEVERITY_FATAL, NULL, NULL);
        } else {
