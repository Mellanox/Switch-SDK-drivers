...
static bool __dev_iterator_reset_info_dump(struct sx_dev *dev, void *context)
{
    struct sx_dev_db_iterator_seq_file_context *dec = (struct sx_dev_db_iterator_seq_file_context*)context;
    struct sx_priv                             *priv = NULL;
    char                                        header[SX_DBG_DUMP_HEADER_MAX_LEN];
    ...
    seq_printf(dec->m, "Last Chip reset type: %d \n",
               priv->reset_info.last_chip_reset_type);

    seq_printf(dec->m, "Last reset duration: %u msec\n",
...
static int __reset_info_dump(struct seq_file *m, void *v, void *context)
{
    ...
    sx_dev_db_iterate(__dev_iterator_reset_info_dump, &dec);
    return 0;
}
...
int __init sx_dbg_init(void)
{
    ...
    sx_dbg_dump_read_handler_register("reset_info_dump", __reset_info_dump, NULL, NULL);
