...
static bool __dev_iterator_cq_dump(struct sx_dev *dev, void *context)
{
    ...
    snprintf(header, HEADER_MAX_LEN, "CQ dump - device_id %u", dev->device_id);
...
static bool __dev_iterator_tele_thrs_dump(struct sx_dev *dev, void *context)
{
    ...
    snprintf(header, HEADER_MAX_LEN, "TELE threshold dump - device_id %u", dev->device_id);
    print_header(dec->m, header);

    print_table_header(dec->m, "%-11s| %-11s| %s", "local_port", "state", "tc_pg_vec");
...
static bool __dev_iterator_monitor_rdq_dump(struct sx_dev *dev, void *context)
{
    ...
    snprintf(header, HEADER_MAX_LEN, "Monitor RDQ dump - device_id %u", dev->device_id);
    print_header(dec->m, header);
...
static bool __dev_iterator_sdq_dump(struct sx_dev *dev, void *context)
{
    ...
    snprintf(header, HEADER_MAX_LEN, "SDQ dump - device_id %u", dev->device_id);
    print_header(dec->m, header);
...
static bool __dev_iterator_rdq_to_filter_ebpf_prog_dump(struct sx_dev *dev, void *context)
{
    ...
    snprintf(header, HEADER_MAX_LEN, "RDQ to filter eBPF program mapping dump - device_id %u", dev->device_id);

    print_header(dec->m, header);
    print_table_header(dec->m, "%-7s| %-7s| %-20s", "#", "RDQ", "Filter eBPF program");
...
static bool __dev_iterator_fw_and_board_info_dump(struct sx_dev *dev, void *context)
{
    ...
    snprintf(header, HEADER_MAX_LEN, "FW and BoardInfo dump - device_id %u", dev->device_id);
    print_header(dec->m, header);
