...
#define SX_PCI_LINK_DOWN_TIME          (60 * 60 * 1000)   /* 60 minutes */
#define SX_PCI_WAIT_AFTER_LINK_UP_TIME (60 * 60 * 1000)   /* 60 minutes */
#define SX_WAIT_WHEN_NO_LNKCAP_DLLLARC (20 * 60 * 1000)   /* 20 minutes */
#else
...
static int __perform_dev_sw_reset(struct sx_dev *dev, u16 asic_id)
{
    ...
    case QUANTUM3_PCI_DEV_ID:
        ...
        if (err) {
            /* in case sdk_sx_reset return unsupported , don't print an error */
            if (err != -EOPNOTSUPP) {
...
int get_system_status(struct sx_dev *dev, u16 *system_status)
{
    ...
    if (unlikely(sx_cr_mode())) {
        err = sx_dpt_cr_space_read(dev->device_id, SX_SYSTEM_STATUS_REG_OFFSET, (unsigned char*)&val, sizeof(val));
        if (err) {
        ...
        sys_status_addr = ioremap(pci_resource_start(dev->pdev, 0) + SX_SYSTEM_STATUS_REG_OFFSET,
