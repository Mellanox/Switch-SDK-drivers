...
static int pci_wait_after_link_up_time = SX_PCI_WAIT_AFTER_LINK_UP_TIME;
static int set_pci_wait_after_link_up_time(const char *val, const struct kernel_param *kp);
static const struct kernel_param_ops pci_wait_after_link_up_time_ops = {
    .set = set_pci_wait_after_link_up_time,
    .get = param_get_int,
};
module_param_cb(pci_wait_after_link_up_time, &pci_wait_after_link_up_time_ops, &pci_wait_after_link_up_time, 0644);
__MODULE_PARM_TYPE(pci_wait_after_link_up_time, "int");
MODULE_PARM_DESC(pci_wait_after_link_up_time,
                 "Time (in ms) to wait for PCI bridge link to be UP, default and minimum are both 500 ms, relevant for PCI disable reset type");
...
static int __pci_link_toggle(struct sx_dev * dev)
{
    ...
    msleep(pci_wait_after_link_up_time);
