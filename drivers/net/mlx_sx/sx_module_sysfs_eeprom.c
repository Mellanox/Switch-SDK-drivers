...
static struct page_limit_bytes_segment cmis_passive_rd_limit_arr[] = {
    {0, 127, 255},
};
...
static int cmis_passive_rd_limit_arr_len = ARRAY_SIZE(cmis_passive_rd_limit_arr);
                                              ...
                                              int                               *limit_arr_len)
{
    ...
    *device_addr = pos;
    ...
    if (is_independent) {
        if ((mgmt_type == EEPROM_MANAGEMENT_TYPE_CMIS) && (access == MODULE_SYSFS_EEPROM_READ) && (flat_mem == 1)) {
            *limit_arr = cmis_passive_rd_limit_arr;
            *limit_arr_len = cmis_passive_rd_limit_arr_len;
        } else {
            *limit_arr_len = 0;
        }
    }
