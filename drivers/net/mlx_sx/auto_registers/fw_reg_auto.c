...
int __MFDE_decode(u8 *outbox, void *ku_reg, void *context)
{
    ...
    mfde_reg->long_process = mlxsw_reg_mfde_long_process_get(outbox);
...
int __PPCNT_encode(u8 *inbox, void *ku_reg, void *context)
{
    ...
    case SXD_PPCNT_GRP_INFINIBAND_PORT_COUNTERS_E:
        mlxsw_reg_ppcnt_qp1_dropped_set(inbox, ppcnt_reg->counter_set.ib_portcntrs_attribute_grp_data.qp1_dropped);
        break;
...
int __PMCR_encode(u8 *inbox, void *ku_reg, void *context)
{
    ...
    mlxsw_reg_pmcr_agc_bw_value_set(inbox, pmcr_reg->agc_bw_value);
...
int __PMCR_decode(u8 *outbox, void *ku_reg, void *context)
{
    ...
    pmcr_reg->agc_bw_value = mlxsw_reg_pmcr_agc_bw_value_get(outbox);
...
int __SLSIR_decode(u8 *outbox, void *ku_reg, void *context)
{
    ...
    slsir_reg->uphy_rev_subminor = mlxsw_reg_slsir_uphy_rev_subminor_get(outbox);
    slsir_reg->bkv_revision_cln = mlxsw_reg_slsir_bkv_revision_cln_get(outbox);
    slsir_reg->bkv_revision_rx = mlxsw_reg_slsir_bkv_revision_rx_get(outbox);
...
int __PDDR_encode(u8 *inbox, void *ku_reg, void *context)
{
    ...
    case SXD_PDDR_PAGE_SELECT_LINK_DOWN_INFO_BUFFER_E:
        mlxsw_reg_pddr_buff_sel_set(inbox, pddr_reg->page_data.pddr_link_down_info_buffer_page.buff_sel);
        for (i = 0; i < SXD_PDDR_LINK_DOWN_BUFFER_NUM; i++) {
            for (j = 0; j < SXD_PDDR_ARRAY512_AUTO_NUM; j++) {
                mlxsw_reg_pddr_array512_auto_set(inbox, j, i, pddr_reg->page_data.pddr_link_down_info_buffer_page.link_down_buffer[i].array512_auto[j]);
            }
        }
        break;
...
int __PDDR_decode(u8 *outbox, void *ku_reg, void *context)
{
    ...
    case SXD_PDDR_PAGE_SELECT_LINK_DOWN_INFO_BUFFER_E:
        pddr_reg->page_data.pddr_link_down_info_buffer_page.buff_sel = mlxsw_reg_pddr_buff_sel_get(outbox);
        for (i = 0; i < SXD_PDDR_LINK_DOWN_BUFFER_NUM; i++) {
            for (j = 0; j < SXD_PDDR_ARRAY512_AUTO_NUM; j++) {
                pddr_reg->page_data.pddr_link_down_info_buffer_page.link_down_buffer[i].array512_auto[j] = mlxsw_reg_pddr_array512_auto_get(outbox, j, i);
            }
        }
        break;
